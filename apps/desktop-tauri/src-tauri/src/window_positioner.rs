...
/// Calculate a panel position beside a floating anchor.
///
/// Anchors in the left half of the work area open to the right; anchors in
/// the right half open to the left. The panel is vertically centred on the
/// anchor and clamped inside the current monitor work area.
pub fn calculate_sidecar_position(
    anchor: &Rect,
    work_area: &Rect,
    panel_size: &PanelSize,
    scale_factor: f64,
) -> (i32, i32) {
    let (panel_width, panel_height) = physical_panel_size(panel_size, scale_factor);
    let anchor_diameter = anchor.width.min(anchor.height) as i32;
    let anchor_center_x = anchor.x + anchor_diameter / 2;
    let work_center_x = work_area.x + work_area.width as i32 / 2;
    let target_x = if anchor_center_x <= work_center_x {
        anchor.x + anchor_diameter - GAP
    } else {
        anchor.x - panel_width - GAP
    };
    let target_y = anchor.y + anchor.height as i32 / 2 - panel_height / 2;

    clamp_to_work_area(target_x, target_y, work_area, panel_size, scale_factor)
}
...
mod tests {
    ...
    #[test]
    fn sidecar_opens_right_of_ball_in_left_half() {
        let work = Rect {
            x: 0,
            y: 0,
            width: 1920,
        ...
        let (x, y) = calculate_sidecar_position(&ball, &work, &panel, 1.0);
    ...
    #[test]
    fn sidecar_ignores_transparent_width_added_by_windows_minimum_size() {
        let work = Rect {
            x: 0,
            y: 0,
            width: 1920,
        ...
        let (x, y) = calculate_sidecar_position(&ball_window, &work, &panel, 1.0);
    ...
    #[test]
    fn sidecar_opens_left_of_ball_in_right_half() {
        let work = Rect {
            x: 0,
            y: 0,
            width: 1920,
        ...
        let (x, _) = calculate_sidecar_position(&ball, &work, &panel, 1.0);

        assert!(x + 320 <= ball.x);
    }

    #[test]
    fn sidecar_clamps_to_negative_coordinate_monitor_edges() {
        let work = Rect {
            x: -1920,
            y: -100,
            width: 1920,
