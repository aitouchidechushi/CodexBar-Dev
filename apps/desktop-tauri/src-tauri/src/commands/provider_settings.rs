...
pub fn reorder_providers(
    ...
    let provider_order = codexbar::settings::normalize_provider_order(&ids);
...
#[tauri::command]
pub fn set_provider_cookie_source(
    app: tauri::AppHandle,
    provider_id: String,
    source: String,
) -> Result<(), String> {
    let (id, source) = validate_provider_cookie_source(&provider_id, &source)?;
...
pub(crate) fn validate_provider_cookie_source(
    provider_id: &str,
    source: &str,
) -> Result<(codexbar::core::ProviderId, String), String> {
...
#[tauri::command]
pub fn get_provider_cookie_source(
    state: tauri::State<'_, Mutex<AppState>>,
    provider_id: String,
) -> Result<Option<String>, String> {
    let settings = crate::storage_services::settings_snapshot(&state);
    Ok(provider_cookie_source_lookup(&settings, &provider_id))
}
...
#[tauri::command]
pub fn set_provider_region(
    app: tauri::AppHandle,
    provider_id: String,
    region: String,
) -> Result<(), String> {
    let (id, region) = validate_provider_region(&provider_id, &region)?;
    crate::storage_services::mutate_settings_from_app(&app, |settings| {
        settings.set_api_region(id, region);
        Ok(())
    })
...
pub(crate) fn validate_provider_region(
    provider_id: &str,
    region: &str,
) -> Result<(codexbar::core::ProviderId, String), String> {
    ...
    let id = region_provider(provider_id)
        .ok_or_else(|| format!("Provider '{provider_id}' does not have a region picker"))?;
    Ok((id, region.to_string()))
}

#[tauri::command]
pub fn get_provider_region(
    state: tauri::State<'_, Mutex<AppState>>,
    provider_id: String,
) -> Result<Option<String>, String> {
    Ok(provider_region_lookup(
...
#[tauri::command]
pub fn set_provider_workspace_id(
    app: tauri::AppHandle,
    provider_id: String,
    workspace_id: String,
) -> Result<(), String> {
    ...
    let has_saved_api_key = load_api_keys_for_runtime()?.has_key(id.cli_name());
    let mut validation_error = None;
    let mutation = crate::storage_services::mutate_settings_from_app(&app, |settings| {
        if let Err(error) = litellm_workspace_change_allowed(
            id,
            ...
            return Err(codexbar::settings::SettingsMutationError::InvalidValue);
        }
    ...
    mutation.map_err(crate::storage_services::command_store_error)?;
    crate::events::emit_settings_changed(&app);
    Ok(())
}
...
#[tauri::command]
pub fn get_provider_workspace_id(
    state: tauri::State<'_, Mutex<AppState>>,
    provider_id: String,
) -> Result<Option<String>, String> {
    ...
    let value = crate::storage_services::settings_snapshot(&state)
        .workspace_id(id)
        .trim()
        .to_string();
...
#[tauri::command]
pub fn get_provider_gateway_url(
    state: tauri::State<'_, Mutex<AppState>>,
    provider_id: String,
) -> Result<Option<String>, String> {
...
#[tauri::command]
pub fn set_provider_gateway_url(
    app: tauri::AppHandle,
    provider_id: String,
    gateway_url: String,
) -> Result<(), String> {
...
pub fn get_provider_cookie_source_options(
    ...
    let lang = crate::storage_services::settings_snapshot(&state).ui_language;
