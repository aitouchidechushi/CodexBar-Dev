...
pub struct ProviderUsageSnapshot {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub credential_display_label: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub credential_display_ordinal: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub credential_group_size: Option<usize>,
...
pub(crate) struct CredentialSnapshotMeta {
    pub id: uuid::Uuid,
...
impl CredentialSnapshotMeta {
    #[cfg(test)]
    pub(crate) fn new(id: uuid::Uuid, display_label: String, display_ordinal: u64) -> Self {
        Self::in_group(id, display_label, display_ordinal, 1)
    }
...
pub(crate) struct ProviderSnapshotIdentity {
    pub provider_id: String,
    pub credential_id: Option<uuid::Uuid>,
}

impl ProviderSnapshotIdentity {
    pub(crate) fn new(provider_id: String, credential_id: Option<uuid::Uuid>) -> Self {
        Self {
            provider_id,
            credential_id,
        }
    ...
    pub(crate) fn from_snapshot(snapshot: &ProviderUsageSnapshot) -> Self {
        Self::new(snapshot.provider_id.clone(), snapshot.credential_id)
    }
}
...
impl ProviderUsageSnapshot {
    pub(crate) fn decorate_for_credential(mut self, credential: &CredentialSnapshotMeta) -> Self {
        self.credential_id = Some(credential.id);
        self.credential_display_label = Some(credential.display_label.clone());
        self.credential_display_ordinal = Some(credential.display_ordinal);
        self.credential_group_size = Some(credential.group_size);
        self
    }
    ...
    pub(super) fn from_fetch_result(
        ...
        let is_minimax_token_plan =
            id == ProviderId::MiniMax && result.source_label == "token-plan";
        ...
        Self {
            ...
            extra_rate_windows: usage
                .extra_rate_windows
                .iter()
                .filter(|extra| id != ProviderId::Kimi || extra.id != "kimi-monthly")
            ...
            account_group_id: usage.account_group_id.clone(),
            account_display_name: usage.account_display_name.clone(),
...
pub struct SettingsSnapshot {
    ...
    start_at_login: bool,
    start_at_login_issue: Option<String>,
    ...
    kimi_monthly_quota_enabled: bool,
    kimi_monthly_consent_accepted: bool,
    kimi_browser_consent_accepted: bool,
...
#[tauri::command]
pub fn get_bootstrap_state(state: tauri::State<'_, std::sync::Mutex<AppState>>) -> BootstrapState {
    bootstrap_state_for(crate::storage_services::settings_snapshot(&state))
}
...
pub fn get_provider_catalog(
    ...
    provider_catalog_for(&crate::storage_services::settings_snapshot(&state))
}
...
pub fn get_settings_snapshot(
    ...
    SettingsSnapshot::from(crate::storage_services::settings_snapshot(&state))
}
...
impl From<Settings> for SettingsSnapshot {
    fn from(settings: Settings) -> Self {
        ...
        let startup = codexbar::install_ownership::current_start_at_login_enabled();
        let (start_at_login, start_at_login_issue) = match startup {
        ...
        Self {
            ...
            kimi_monthly_quota_enabled: settings.kimi_monthly_quota_enabled,
            kimi_monthly_consent_accepted: settings.kimi_monthly_consent_accepted,
            kimi_browser_consent_accepted: settings.kimi_browser_consent_accepted,
