...
use codexbar::core::{ProviderId, TokenAccount, TokenAccountMutationError, TokenAccountSupport};
use serde::Serialize;
use std::sync::Mutex;
...
pub fn get_token_accounts(
    ...
    let repository = crate::storage_services::storage_snapshot(&state).token_accounts;
    let data = repository
        .load_provider(id)
...
pub fn remove_token_account(
    ...
    let data = repository
        .mutate_provider(id, |data| {
            if data.remove_account(uuid).is_none() {
