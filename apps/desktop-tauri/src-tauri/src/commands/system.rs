...
pub(crate) fn runtime_identity_from_state(
    state: &Mutex<AppState>,
) -> Result<crate::runtime_identity::RuntimeIdentity, String> {
    state
        .lock()
        .map(|state| state.runtime_identity.clone())
        .map_err(|_| "Runtime identity state is unavailable".to_string())
}

#[tauri::command]
pub fn get_runtime_identity(
    state: tauri::State<'_, Mutex<AppState>>,
) -> Result<crate::runtime_identity::RuntimeIdentity, String> {
    runtime_identity_from_state(state.inner())
}

#[tauri::command]
pub fn get_app_info(state: tauri::State<'_, Mutex<AppState>>) -> AppInfoBridge {
...
#[tauri::command]
pub fn play_notification_sound(state: tauri::State<'_, Mutex<AppState>>) -> Result<(), String> {
...
pub fn quit_app(app: tauri::AppHandle) {
    ...
    let update_attempt = if settings.install_updates_on_quit {
        app.try_state::<std::sync::Mutex<crate::state::AppState>>()
            .map(|state| super::updater::apply_ready_update(&state))
    } else {
        None
    };
    finish_quit(settings.install_updates_on_quit, update_attempt, |reason| {
...
fn finish_quit(
    ...
    update_attempt: Option<Result<(), String>>,
    request_exit: impl FnOnce(crate::app_exit::ExitReason),
) {
    if let Some(Err(error)) = update_attempt {
        tracing::debug!("install-on-quit skipped: {error}");
    }
    let reason = if install_updates_on_quit {
        crate::app_exit::ExitReason::InstallUpdatesOnQuit
    } else {
        crate::app_exit::ExitReason::FrontendQuit
    };
    request_exit(reason);
}

fn dashboard_url_for_provider(settings: &Settings, provider_id: &str) -> Option<String> {
...
pub fn open_provider_dashboard(
    ...
    let settings = crate::storage_services::settings_snapshot(&state);
    let url = dashboard_url_for_provider(&settings, &provider_id)
...
pub async fn trigger_provider_login(
    ...
    let settings = crate::storage_services::settings_snapshot_from_app(&app);
    if let Some(url) = dashboard_url_for_provider(&settings, &provider_id) {
...
async fn run_copilot_device_login(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let repository = crate::storage_services::storage_snapshot_from_app(app).token_accounts;
    repository
        .mutate_provider(ProviderId::Copilot, |data| {
...
mod tests {
    ...
    fn dashboard_url_resolves_from_codex_provider_metadata() {
        assert_eq!(
            dashboard_url_for_provider(&Settings::default(), "codex").as_deref(),
            Some("https://chatgpt.com/codex/settings/usage")
        );
    }

    #[test]
    fn frontend_quit_requests_the_frontend_normal_exit_reason() {
        let requested = Cell::new(None);

        finish_quit(false, None, |reason| requested.set(Some(reason)));

        assert_eq!(
            requested.get(),
            Some(crate::app_exit::ExitReason::FrontendQuit)
        );
    }

    #[test]
    fn failed_install_on_quit_still_requests_normal_exit() {
        let requested = Cell::new(None);

        finish_quit(
            true,
            Some(Err("installer launch failed".to_string())),
            |reason| requested.set(Some(reason)),
        );

        assert_eq!(
            requested.get(),
            Some(crate::app_exit::ExitReason::InstallUpdatesOnQuit)
        );
    }

    #[test]
    fn successful_install_on_quit_requests_normal_exit_exactly_once() {
        let requested = Cell::new(None);
        let request_count = Cell::new(0);

        finish_quit(true, Some(Ok(())), |reason| {
            request_count.set(request_count.get() + 1);
            requested.set(Some(reason));
        });
