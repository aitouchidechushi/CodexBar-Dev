...
//! every provider. Legacy OpenAI dashboard cache data is deliberately not used:
//! no current writer maintains it, so reading it would let an older program's
//! stale plaintext cache appear as current chart data.
...
pub async fn get_provider_chart_data(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        build_provider_chart_data_with_cancel(provider_id, &settings, Some(cancel))
    })
    .await
    .unwrap_or_else(|err| {
...
pub async fn get_provider_local_usage_summary(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        load_provider_local_usage_summary(&provider_id, &settings)
    })
    .await
    .unwrap_or_else(|err| {
...
pub(crate) fn build_provider_chart_data(
    ...
    build_provider_chart_data_with_cancel(provider_id, &Settings::default(), None)
}
...
fn build_provider_chart_data_with_cancel(
    ...
    let raw_cost = get_daily_cost_history(&provider_id, 30, settings);
    let cost_history: Vec<DailyCostPoint> = raw_cost
        .into_iter()
        .map(|(date, value)| DailyCostPoint { date, value })
        .collect();

    let credits_history = Vec::new();
    let usage_breakdown = Vec::new();
        ...
        .is_some_and(|flag| flag.load(Ordering::Relaxed))
    {
        ...
        load_local_usage_summary_cached(&provider_id, settings, cancel.as_deref())
    };
...
fn load_local_usage_summary(
    ...
    load_local_usage_summary_with_unknown_models(provider_id, settings, cancel).0
}
...
fn load_local_usage_summary_with_unknown_models(
    ...
    let Some(thirty_day) = scan_local_cost(provider_id, 30, settings, cancel) else {
        return (None, HashSet::new());
    };
    if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
        return (None, HashSet::new());
    }
    let today = scan_local_cost(provider_id, 1, settings, cancel).unwrap_or_default();
...
pub(crate) fn load_provider_local_usage_summary(
    ...
    load_local_usage_summary_cached(provider_id, settings, None)
}
...
pub(crate) async fn refresh_provider_local_usage_cache(
    ...
    for (provider_id, mut summary, unknown_models) in scans {
                ...
                .await
        {
            ...
            let rescan_settings = settings.clone();
            summary = tauri::async_runtime::spawn_blocking(move || {
                load_local_usage_summary(&rescan_provider, &rescan_settings, None)
            })
            .await
            .unwrap_or(summary);
        }
...
fn load_local_usage_summary_cached(
    ...
    let summary = load_local_usage_summary(provider_id, settings, cancel);
    if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
        return None;
    }
...
fn scan_local_cost(
    ...
    let scanner = CostScanner::new(days, settings);
    match provider_id {
