...
pub struct SettingsUpdate {
    ...
    pub floating_ball_enabled: Option<bool>,
    pub kimi_monthly_quota_enabled: Option<bool>,
    pub kimi_monthly_consent_accepted: Option<bool>,
    pub kimi_browser_consent_accepted: Option<bool>,
...
impl SettingsUpdate {
    fn refreshes_provider_data(&self) -> bool {
        self.enabled_providers.is_some() || self.kimi_monthly_quota_enabled.is_some()
    }

    fn invalidates_local_usage_cache(&self) -> bool {
        self.codex_custom_sessions_dirs.is_some() || self.ui_language.is_some()
    }
    ...
    fn notifies_float_bar(&self) -> bool {
        self.enabled_providers.is_some()
            ...
            || self.kimi_monthly_quota_enabled.is_some()
    }
    ...
    fn validate_shortcut_input(&self) -> Result<(), String> {
        let Some(new_shortcut) = &self.global_shortcut else {
            return Ok(());
        };

        if !new_shortcut.trim().is_empty()
            && crate::shortcut_bridge::parse_shortcut(new_shortcut).is_none()
        {
            return Err(format!(
                "Invalid shortcut \"{new_shortcut}\". Use a combination like Ctrl+Shift+U."
            ));
        }
    ...
    fn apply_general_settings(self, settings: &mut Settings) -> Self {
        // Startup is owned by Windows, not this settings document. The request
        // is executed separately and snapshots read back the actual Run state.
        if let Some(v) = self.start_minimized {
            settings.start_minimized = v;
        }
        if let Some(v) = self.floating_ball_enabled {
            settings.floating_ball_enabled = v;
        }
        if let Some(v) = self.kimi_monthly_quota_enabled {
            settings.kimi_monthly_quota_enabled = v;
        }
        if let Some(v) = self.kimi_monthly_consent_accepted {
            settings.kimi_monthly_consent_accepted = v;
        }
        if let Some(v) = self.kimi_browser_consent_accepted {
            settings.kimi_browser_consent_accepted = v;
        }
    ...
    fn apply_to(self, settings: &mut Settings) -> crate::floatbar::SettingsPatch {
...
pub fn update_settings(
    ...
    let effect_lock = crate::storage_services::settings_effect_lock_from_app(&app);
    let _effect_guard = effect_lock
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let previous = crate::storage_services::settings_snapshot_from_app(&app);
    let notify_float_bar = patch.notifies_float_bar();
    let floating_ball_changed = patch.floating_ball_enabled.is_some();
    let kimi_monthly_disabled = patch.kimi_monthly_quota_enabled == Some(false);
    let refresh_provider_data = patch.refreshes_provider_data();
    let clear_local_usage_cache = patch.invalidates_local_usage_cache();
    ...
    let previous_promoted = previous.promote_tray_icon;
    let previous_language = previous.ui_language;
    let previous_shortcut = previous.global_shortcut.clone();
    let start_at_login_changed = patch.start_at_login;

    patch.validate_shortcut_input()?;

    let kimi_commit_guard = if kimi_monthly_disabled {
        Some(crate::kimi_accounts::lock_account_commit()?)
    } else {
        None
    };
    let patch_for_transaction = patch.clone();
    let (float_bar_patch, settings) =
        crate::storage_services::mutate_settings_from_app(&app, move |settings| {
            Ok(patch_for_transaction.apply_to(settings))
        })
        .map_err(crate::storage_services::command_store_error)?;
    codexbar::core::configure_app_proxy(&settings);

    let mut post_commit_errors = Vec::new();
    if let Some(enabled) = start_at_login_changed
        && let Err(error) = Settings::apply_start_at_login_registry(enabled)
    {
        post_commit_errors.push(format!(
            "Failed to update start-at-login ownership: {error}"
        ));
    }
    if let Some(new_shortcut) = patch.global_shortcut.as_deref()
        && new_shortcut != previous_shortcut
    {
        if new_shortcut.trim().is_empty() {
            if let Err(error) =
                crate::shortcut_bridge::unregister_shortcut(&app, &previous_shortcut)
            {
                post_commit_errors.push(error);
            }
        } else {
            if let Err(error) =
                crate::shortcut_bridge::reregister_shortcut(&app, &previous_shortcut, new_shortcut)
            {
                post_commit_errors.push(error);
            }
        }
    ...
    if kimi_monthly_disabled {
        crate::kimi_accounts::disable_kimi_accounts(&app);
    }
    drop(kimi_commit_guard);
    ...
    if floating_ball_changed {
        crate::floating_ball::apply_enabled(&app, settings.floating_ball_enabled);
    }
    ...
    if !post_commit_errors.is_empty() {
        return Err(post_commit_errors.join("; "));
    }
...
mod tests {
    ...
    #[test]
    fn autostart_request_does_not_persist_an_unverified_windows_state() {
    ...
    fn only_data_affecting_settings_refresh_providers() {
        ...
        assert!(
            SettingsUpdate {
                kimi_monthly_quota_enabled: Some(true),
                ..Default::default()
            }
            .refreshes_provider_data()
        );
    ...
    #[test]
    fn local_usage_cache_is_invalidated_by_paths_or_language() {
        assert!(
            SettingsUpdate {
                codex_custom_sessions_dirs: Some(vec!["D:\\codex".to_string()]),
                ..Default::default()
            }
            .invalidates_local_usage_cache()
        );
        assert!(
            SettingsUpdate {
                ui_language: Some("japanese".to_string()),
    ...
    #[test]
    fn kimi_monthly_settings_update_both_persisted_flags() {
        let mut settings = Settings::default();
        SettingsUpdate {
            kimi_monthly_quota_enabled: Some(true),
            kimi_monthly_consent_accepted: Some(true),
            ..Default::default()
        }
        .apply_to(&mut settings);

        assert!(settings.kimi_monthly_quota_enabled);
        assert!(settings.kimi_monthly_consent_accepted);
        let snapshot = serde_json::to_value(SettingsSnapshot::from(settings)).unwrap();
        assert_eq!(snapshot["kimiMonthlyQuotaEnabled"], true);
        assert_eq!(snapshot["kimiMonthlyConsentAccepted"], true);
    }

    #[test]
    fn kimi_browser_consent_update_is_separate_and_idempotent() {
        let mut settings = Settings {
            kimi_monthly_quota_enabled: true,
            kimi_monthly_consent_accepted: true,
            kimi_browser_consent_accepted: false,
            refresh_interval_secs: 300,
            adaptive_refresh: true,
            ..Settings::default()
        };
        let apply = || SettingsUpdate {
            kimi_browser_consent_accepted: Some(true),
            ..Default::default()
        };

        apply().apply_general_settings(&mut settings);
        apply().apply_general_settings(&mut settings);

        assert!(settings.kimi_browser_consent_accepted);
        assert!(settings.kimi_monthly_consent_accepted);
        assert!(settings.kimi_monthly_quota_enabled);
        assert_eq!(settings.refresh_interval_secs, 300);
        assert!(settings.adaptive_refresh);
        assert!(!apply().refreshes_provider_data());
    }
