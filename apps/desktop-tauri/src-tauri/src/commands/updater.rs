...
pub fn apply_update(
    ...
    apply_update_then_request_exit(
        || apply_ready_update(&state),
        || {
            crate::app_exit::request_normal_exit(&app, crate::app_exit::ExitReason::ApplyUpdate);
        },
    )
}

fn apply_update_then_request_exit(
    apply_update: impl FnOnce() -> Result<(), String>,
...
mod tests {
    ...
    #[test]
    fn successful_update_launch_requests_normal_exit() {
        let exit_requested = Cell::new(false);

        let result = apply_update_then_request_exit(|| Ok(()), || exit_requested.set(true));
    ...
    #[test]
    fn failed_update_launch_returns_error_without_requesting_exit() {
        let exit_requested = Cell::new(false);

        let result = apply_update_then_request_exit(
            || Err("installer launch failed".to_string()),
            || exit_requested.set(true),
        );

        assert_eq!(result, Err("installer launch failed".to_string()));
        assert!(!exit_requested.get());
    }
}
