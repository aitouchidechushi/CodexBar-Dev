...
use crate::commands::{ProviderSnapshotIdentity, ProviderUsageSnapshot};
use crate::proof_harness::ProofConfig;
use crate::runtime_identity::RuntimeIdentity;
use crate::storage_services::StorageServices;
...
pub struct AppState {
    ...
    pub settings: codexbar::settings::Settings,
    ...
    pub transient_provider_failure_counts: HashMap<ProviderSnapshotIdentity, u8>,
...
impl AppState {
    ...
    #[cfg(test)]
    pub fn new() -> Self {
        static TEST_RUNTIME_IDENTITY: std::sync::OnceLock<RuntimeIdentity> =
            std::sync::OnceLock::new();
        let runtime_identity = TEST_RUNTIME_IDENTITY
            .get_or_init(|| {
                RuntimeIdentity::capture_current(chrono::Utc::now())
                    .expect("test executable must have a readable runtime identity")
            })
            .clone();
        let fixture_root = std::env::temp_dir().join("codexbar-app-state-unwritten-test-fixture");
        let storage = StorageServices::new(
            codexbar::storage::StoragePaths::from_roots(
                fixture_root.join("roaming"),
                fixture_root.join("local"),
            ),
            codexbar::storage::UserScopeId::from_stable_identifier(b"app-state-test-user"),
        );
