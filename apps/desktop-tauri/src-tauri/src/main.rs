...
use state::AppState;
use storage_services::{StartupStorageState, StorageServices};
...
fn should_suppress_blur_dismiss(force_start_visible: bool, proof_mode: bool) -> bool {
    force_start_visible || proof_mode
}

fn main() {
    let process_started_at = chrono::Utc::now();
    codexbar::logging::init(false, false).expect("failed to initialize logging");
    let runtime_identity = runtime_identity::RuntimeIdentity::capture_current(process_started_at)
        .unwrap_or_else(|error| runtime_identity::report_fatal_startup_error(error));
    tracing::info!(
        pid = runtime_identity.pid,
        version = %runtime_identity.build.semantic_version,
        commit = %runtime_identity.build.git_commit,
        channel = ?runtime_identity.build.build_channel,
        executable_sha256 = %runtime_identity.executable_sha256,
        "captured runtime executable identity"
    );
    ...
    let startup_decision = startup_ownership::preflight_current_process(&runtime_identity)
        .unwrap_or_else(|error| startup_ownership::report_preflight_failure(error));
    match &startup_decision {
        startup_ownership::StartupDecision::Continue => {
            tracing::info!("startup ownership preflight allows initialization");
        }
        startup_ownership::StartupDecision::HandOffToSameOrNewer { pid } => {
            // The single-instance plugin is registered before application setup
            // below. It will forward this launch and exit before any storage is
            // opened in the secondary process.
            tracing::info!(pid, "startup ownership will hand off to the existing instance");
        }
        startup_ownership::StartupDecision::RequestVerifiedOldExit { .. }
        | startup_ownership::StartupDecision::BlockUnknownConflict { .. } => {
            startup_ownership::report_startup_conflict(&startup_decision)
        }
    }
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::update_api_key_label,
            commands::replace_api_key_secret,
            commands::delete_api_key,
            commands::reorder_api_keys,
            ...
            commands::open_path,
            kimi_accounts::list_kimi_accounts,
            kimi_accounts::begin_kimi_account_login,
            kimi_accounts::complete_kimi_account_login,
            kimi_accounts::cancel_kimi_account_login,
            kimi_accounts::refresh_kimi_browser_accounts,
            kimi_accounts::remove_kimi_account,
            tray_visibility::tray_visibility_status,
            floatbar::show_float_bar,
            floatbar::complete_float_bar_initial_show,
            ...
            tray_hover::set_tray_hover_pointer_inside,
            tray_hover::resize_tray_hover,
            floating_ball::toggle_floating_quota,
            floating_ball::hide_floating_quota,
            floating_ball::resize_floating_quota,
            floating_ball::start_floating_quota_drag,
            floating_ball::toggle_floating_quota_topmost,
            floating_ball::toggle_floating_quota_desktop,
            floating_ball::open_main_window_from_floating_quota,
        ])
        .setup(move |app| {
            // Tauri initializes registered plugins before this callback. In
            // particular, the single-instance boundary is now owned before
            // StorageV2 is discovered or any user settings are read.
            let storage = StorageServices::discover()
                .unwrap_or_else(|error| storage_services::report_fatal_discovery_error(error));
            let settings = match storage.load_startup() {
                StartupStorageState::Ready { settings } => *settings,
                StartupStorageState::RecoveryRequired { issues } => {
                    storage_services::report_fatal_startup_error(issues)
                }
            };
            ...
            let mut initial_state =
                AppState::with_runtime_identity(runtime_identity, storage, settings.clone());
            ...
            floating_ball::install(app.handle(), settings.floating_ball_enabled);
            kimi_accounts::install(
                app.handle().clone(),
        ...
        .on_window_event(move |window, event| {
            if kimi_accounts::handle_window_event(window, event) {
                return;
            }
            ...
            {
                kimi_accounts::cancel_active_login_in_background(window.app_handle(), true, true);
            }
...
mod tests {
    ...
    fn automation_launch_opens_and_suppresses_blur_dismiss() {
        ...
        assert!(should_suppress_blur_dismiss(
            launch.suppress_blur_dismiss,
            false
        ));
    }
