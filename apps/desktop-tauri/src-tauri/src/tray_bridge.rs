...
#[derive(Debug, Clone)]
pub(crate) struct ProviderPresentationSummary {
    pub(crate) provider_id: String,
    pub(crate) display_name: String,
    pub(crate) snapshots: Vec<crate::commands::ProviderUsageSnapshot>,
    pub(crate) credential_count: usize,
}

impl ProviderPresentationSummary {
    pub(crate) fn is_multi_key(&self) -> bool {
        self.credential_count > 1
    }

    pub(crate) fn is_all_failed(&self) -> bool {
        !self.snapshots.is_empty()
            && self
                .snapshots
                .iter()
                .all(|snapshot| snapshot.error.is_some())
    }

    pub(crate) fn successful_snapshot(&self) -> Option<&crate::commands::ProviderUsageSnapshot> {
        self.snapshots
            .iter()
            .find(|snapshot| snapshot.error.is_none())
    }
...
pub(crate) fn provider_presentation_summaries(
    snapshots: &[crate::commands::ProviderUsageSnapshot],
) -> Vec<ProviderPresentationSummary> {
    let mut summaries = Vec::<ProviderPresentationSummary>::new();
    let mut indices = std::collections::HashMap::<String, usize>::new();

    for snapshot in snapshots {
        let provider_id = ProviderId::from_cli_name(&snapshot.provider_id)
            .map(|provider| provider.cli_name().to_string())
            .unwrap_or_else(|| snapshot.provider_id.clone());
        let index = if let Some(index) = indices.get(&provider_id) {
            *index
        } else {
            let index = summaries.len();
            indices.insert(provider_id.clone(), index);
            summaries.push(ProviderPresentationSummary {
                provider_id,
        ...
        if snapshot.credential_id.is_some() {
            summary.credential_count += 1;
        }
        if let Some(group_size) = snapshot.credential_group_size {
            summary.credential_count = summary.credential_count.max(group_size);
        }
...
pub fn setup(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    let settings = crate::storage_services::settings_snapshot_from_app(app.handle());
    let menu = build_native_tray_menu(
        app.handle(),
        &crate::commands::provider_catalog_for(&settings),
        &[],
        &settings,
    )?;
            ...
            match event {
                TrayIconEvent::Enter { position, rect, .. } => {
                    ...
                    if let Some(anchor) = anchor {
                        let _ = crate::tray_hover::show(app, anchor);
                    }
                }
                TrayIconEvent::Leave { .. } => crate::tray_hover::schedule_hide(app),
                TrayIconEvent::Click {
                    button,
                    button_state,
                    position,
                    ...
                    crate::tray_hover::schedule_hide(app);
                    store_anchor(app, &rect, position);
    ...
    let promote = settings.promote_tray_icon;
    if promote {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            let still_promote =
                crate::storage_services::settings_snapshot_from_app(&app_handle).promote_tray_icon;
            if still_promote {
                crate::tray_visibility::apply_promotion(true);
            }
            drop(app_handle);
...
fn handle_menu_event(app: &AppHandle, id: &str) {
    match resolve_menu_action(id) {
        ...
        Some(MenuAction::ToggleProvider(provider_id)) => {
            let result = crate::storage_services::mutate_settings_from_app(app, |settings| {
            ...
            if let Err(error) = result {
                let _ = crate::storage_services::command_store_error(error);
                return;
            }
        ...
        Some(MenuAction::Quit) => {
            request_tray_quit(|reason| {
                crate::app_exit::request_normal_exit(app, reason);
            });
        }
        None => {}
    }
...
fn request_tray_quit(request_exit: impl FnOnce(crate::app_exit::ExitReason)) {
    request_exit(crate::app_exit::ExitReason::TrayQuit);
}

/// Rebuild the native tray menu from current provider + settings state.
pub(crate) fn rebuild_tray_menu(app: &AppHandle) {
    let (settings, cached_snapshots) = if let Some(st) = app.try_state::<Mutex<AppState>>() {
        let guard = st.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        (guard.settings.clone(), guard.provider_cache.clone())
    } else {
        return;
    };
    let catalog = crate::commands::provider_catalog_for(&settings);
    let status_labels = {
        let snapshots =
            presentation_snapshots(&cached_snapshots, settings.codex_spark_usage_visible());
        status_labels_for_settings(&settings, &snapshots, settings.ui_language)
    };
    if let Ok(menu) = build_native_tray_menu(app, &catalog, &status_labels, &settings)
...
/// Update the tray icon pixels while keeping the native tooltip short.
///
...
pub fn update_tray_icon_and_tooltip(
    ...
    let settings = crate::storage_services::settings_snapshot_from_app(app);
    let snapshots = presentation_snapshots(snapshots, settings.codex_spark_usage_visible());
    let (rgba, w, h) = match choose_tray_icon_plan(&settings, &snapshots) {
        TrayIconPlan::Usage {
            session_percent,
            weekly_percent,
        } => render_tray_icon_for_settings(&settings, session_percent, weekly_percent, false),
        TrayIconPlan::Neutral => render_neutral_tray_icon(false),
        TrayIconPlan::NeutralError => render_neutral_tray_icon(true),
        TrayIconPlan::Error => render_tray_icon_for_settings(&settings, 0.0, None, true),
        TrayIconPlan::Empty => render_tray_icon_for_settings(&settings, 0.0, None, false),
    };
    let icon = Image::new_owned(rgba, w, h);
    let _ = tray.set_icon(Some(icon));

    // ── Tooltip ───────────────────────────────────────────────────────────
    // Windows truncates native tray tooltips to a fixed-size UTF-16 buffer.
    // The provider-generic details live in our scrollable hover window.
    let _ = tray.set_tooltip(Some("CodexBar"));
}
...
fn status_labels_for_settings(
    ...
    let summaries = ordered_provider_summaries(settings, snapshots);
    if settings.tray_icon_mode == TrayIconMode::PerProvider {
        return summaries
            .into_iter()
            .filter(|summary| summary.is_multi_key() || summary.successful_snapshot().is_some())
            .map(|summary| provider_summary_status_label(&summary, lang))
            .collect::<Vec<_>>();
    }

    if let Some(summary) = summaries
        .iter()
        .find(|summary| summary.is_multi_key() && !summary.is_all_failed())
    {
        let (_, label) = provider_summary_status_label(summary, lang);
        return vec![("status_summary".to_string(), label)];
    }

    let healthy = summaries
        .iter()
        .filter_map(ProviderPresentationSummary::successful_snapshot)
        .collect::<Vec<_>>();
    if let Some(selected) = pick_tray_provider(
        &healthy,
    ...
    summaries
        .iter()
        .find(|summary| summary.is_multi_key())
        .map(|summary| {
            let (_, label) = provider_summary_status_label(summary, lang);
...
fn ordered_provider_summaries(
    ...
    let mut summaries = provider_presentation_summaries(snapshots);
    summaries.sort_by(
        |a, b| match (order.get(&a.provider_id), order.get(&b.provider_id)) {
...
fn provider_summary_status_label(
    summary: &ProviderPresentationSummary,
    lang: codexbar::settings::Language,
) -> (String, String) {
    use codexbar::locale::{LocaleKey, format_locale, get_text};

    let status = if summary.is_all_failed() {
        get_text(lang, LocaleKey::TrayStatusRowError)
    } else if summary.is_multi_key() {
        format_locale(
            lang,
            LocaleKey::ApiKeyCount,
            &[&summary.credential_count.to_string()],
        )
    } else if let Some(snapshot) = summary.successful_snapshot() {
        return provider_status_label(snapshot, lang);
    } else {
        get_text(lang, LocaleKey::TrayStatusRowError)
    };
    (
        summary.provider_id.clone(),
        format!("{} {status}", summary.display_name),
    )
}
...
fn choose_tray_icon_plan(
    settings: &Settings,
    snapshots: &[crate::commands::ProviderUsageSnapshot],
) -> TrayIconPlan {
    let summaries = ordered_provider_summaries(settings, snapshots);
    if summaries.is_empty() {
        return TrayIconPlan::Empty;
    }
    if summaries
        .iter()
        .any(|summary| summary.is_multi_key() && !summary.is_all_failed())
    {
        return TrayIconPlan::Neutral;
    }
    ...
    if let Some(snapshot) = pick_tray_provider(&healthy, prefer_highest) {
        let (session_percent, weekly_percent) = selected_tray_percents(snapshot, settings);
        return TrayIconPlan::Usage {
            session_percent,
            weekly_percent,
        };
    }
    ...
    if summaries
        .iter()
        .any(ProviderPresentationSummary::is_multi_key)
    {
        TrayIconPlan::NeutralError
    } else {
        TrayIconPlan::Error
    }
}

fn render_neutral_tray_icon(has_error: bool) -> (Vec<u8>, u32, u32) {
    const SIZE: u32 = 32;
    ...
    for y in 2..SIZE - 2 {
        for x in 2..SIZE - 2 {
            ...
            rgba[index..index + 4].copy_from_slice(&background);
        }
    }
    for (y_start, y_end) in [(9, 14), (18, 23)] {
        for y in y_start..y_end {
            for x in 7..SIZE - 7 {
                let index = ((y * SIZE + x) * 4) as usize;
                rgba[index..index + 4].copy_from_slice(&foreground);
            }
        }
    }
    (rgba, SIZE, SIZE)
...
fn build_tooltip(
    ...
    let summaries = provider_presentation_summaries(snapshots);
    let mut lines = Vec::with_capacity(summaries.len() + 1);
    for summary in summaries {
        let status = if summary.is_multi_key() {
            if summary.is_all_failed() {
                format!("{}: {}", summary.display_name, error_label)
            } else {
                let count = codexbar::locale::format_locale(
                    lang,
                    LocaleKey::ApiKeyCount,
                    &[&summary.credential_count.to_string()],
                );
                format!("{}: {count}", summary.display_name)
            }
        } else if let Some(s) = summary.snapshots.first() {
...
mod tests {
    ...
    #[test]
    fn tray_quit_requests_the_tray_normal_exit_reason() {
        let requested = std::cell::Cell::new(None);

        request_tray_quit(|reason| requested.set(Some(reason)));

        assert_eq!(requested.get(), Some(crate::app_exit::ExitReason::TrayQuit));
    }
    ...
    fn credential_snapshot(
        ...
        let mut snapshot = fake_snapshot(id, display, used_percent);
        snapshot.credential_id = Some(uuid::Uuid::parse_str(credential_id).unwrap());
        snapshot.credential_display_label = Some("safe label".to_string());
        snapshot.credential_display_ordinal = Some(1);
    ...
    #[test]
    fn multi_key_status_and_tooltip_are_one_localized_neutral_provider_row() {
        let settings = Settings {
            tray_icon_mode: TrayIconMode::PerProvider,
            ..Settings::default()
        };
        ...
        let tooltip = build_tooltip(&snapshots, codexbar::settings::Language::English);

        assert_eq!(
            labels,
            vec![("codex".to_string(), "Codex 2 keys".to_string())]
        );
    ...
    #[test]
    fn multi_key_tray_icon_plan_is_neutral_without_a_percentage() {
        ...
        assert_eq!(
            choose_tray_icon_plan(&settings, &snapshots),
            TrayIconPlan::Neutral,
        );
    }
    ...
    fn retained_sibling_stays_neutral_while_changed_key_refreshes() {
        let settings = Settings::default();
        let mut sibling = credential_snapshot(
            "codex",
            "Codex",
            91.0,
            "00000000-0000-0000-0000-000000000001",
        ...
        assert_eq!(
            ...
            vec![("status_summary".to_string(), "Codex 2 keys".to_string())],
        );
    }

    #[test]
    fn partial_multi_key_failure_stays_neutral_and_hides_raw_error() {
        let settings = Settings {
            tray_icon_mode: TrayIconMode::PerProvider,
            ..Settings::default()
        };
        ...
        assert_eq!(tooltip, "CodexBar\nCodex: 2 keys");
        assert!(!tooltip.contains("84%"));
        assert!(!tooltip.contains("secret backend failure"));
    }

    #[test]
    fn all_failed_credentials_keep_one_safe_provider_error_row() {
        let settings = Settings {
            tray_icon_mode: TrayIconMode::PerProvider,
            ..Settings::default()
        };
        ...
        assert_eq!(
            labels,
            vec![("codex".to_string(), "Codex Error".to_string())]
        );
        assert_eq!(tooltip, "CodexBar\nCodex: Error");
    }
