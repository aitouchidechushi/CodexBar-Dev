...
pub(crate) use window::apply_no_activate;
...
pub fn install(app: &tauri::AppHandle) {
    ...
    let persisted = crate::storage_services::settings_snapshot_from_app(app);
    ...
    match event {
        tauri::WindowEvent::Moved(_) | tauri::WindowEvent::Resized(_) => {
            ...
            let relocated = if window::is_off_all_monitors(window) {
                let style =
                    crate::storage_services::settings_snapshot_from_app(window.app_handle())
                        .float_bar_style;
...
pub fn toggle(app: &tauri::AppHandle) {
    let settings = match crate::storage_services::mutate_settings_from_app(app, |settings| {
