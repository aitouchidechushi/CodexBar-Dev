...
struct FloatBarShowEffects {
    initial_visibility: Option<bool>,
    show_after_prepare: bool,
    activate_topmost_guard: bool,
}

fn floatbar_show_effects(window_exists: bool) -> FloatBarShowEffects {
    if window_exists {
        FloatBarShowEffects {
            initial_visibility: None,
            show_after_prepare: true,
            activate_topmost_guard: true,
        }
    } else {
        FloatBarShowEffects {
            initial_visibility: Some(false),
            show_after_prepare: false,
            activate_topmost_guard: false,
        }
    }
}
...
pub fn show(
    ...
    let existing_window = app.get_webview_window(FLOATBAR_LABEL);
    let effects = floatbar_show_effects(existing_window.is_some());
    if let Some(window) = existing_window {
        ...
        if effects.show_after_prepare {
            window.show().map_err(|e| e.to_string())?;
            apply_always_on_top(&window);
        }
        if effects.activate_topmost_guard {
    ...
    let win = builder
        ...
        .visible(effects.initial_visibility.unwrap_or(false))
        .build()
...
mod tests {
    ...
    #[test]
    fn first_show_request_builds_hidden_without_showing_or_activating_guard() {
        assert_eq!(
            floatbar_show_effects(false),
            FloatBarShowEffects {
                initial_visibility: Some(false),
                show_after_prepare: false,
                activate_topmost_guard: false,
    ...
    #[test]
    fn later_show_request_shows_existing_window_and_activates_guard() {
        assert_eq!(
            floatbar_show_effects(true),
            FloatBarShowEffects {
                initial_visibility: None,
                show_after_prepare: true,
                activate_topmost_guard: true,
