...
pub fn open_or_focus(app: &tauri::AppHandle, tab: &str) -> Result<(), String> {
    ...
    let window = build(app, true, tab)?;
    window.set_focus().map_err(|e| e.to_string())?;
    Ok(())
}

/// Ensure the Settings host exists without showing or focusing it.
/// Background Kimi refresh webviews use this host and remain invisible.
pub fn ensure_hidden(app: &tauri::AppHandle) -> Result<tauri::WebviewWindow, String> {
    if let Some(window) = app.get_webview_window(SETTINGS_LABEL) {
        return Ok(window);
    }
    build(app, false, "kimiAccounts")
}

fn build(app: &tauri::AppHandle, visible: bool, tab: &str) -> Result<tauri::WebviewWindow, String> {
...
pub fn dismiss(app: &tauri::AppHandle, window: &tauri::WebviewWindow) -> Result<(), String> {
    crate::kimi_accounts::cancel_active_login_in_background(app, true, true);
