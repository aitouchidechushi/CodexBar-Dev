...
use crate::commands::{ProviderCatalogEntry, provider_catalog_for};
use crate::events;
use crate::shell;
...
fn execute_proof_command(
    ...
    match command {
        ...
        ProofCommand::OpenNativeMenu => open_proof_native_menu(app),
...
fn open_proof_native_menu(app: &AppHandle) -> Result<ProofCommandOutcome, String> {
    let settings = crate::storage_services::settings_snapshot_from_app(app);
    let (menu_path, menu_items) = native_menu_snapshot_for_path(&settings, "tray");
...
fn persist_about_path_snapshot(
    settings: &codexbar::settings::Settings,
    result: Result<(), String>,
) -> Result<(), String> {
    persist_about_path_snapshot_for_item(settings, "about", result)
}
...
fn persist_about_path_snapshot_for_item(
    ...
    match result {
        Ok(()) => {
            let (menu_path, menu_items) = match native_menu_context_for_item(settings, item_id) {
                Ok(context) => context,
                Err(err) => {
                    clear_menu_snapshot();
                    return Err(err);
...
fn native_menu_snapshot_for_path(
    settings: &codexbar::settings::Settings,
    menu_path: &str,
) -> (String, Vec<String>) {
    let providers = provider_catalog_for(settings);
    native_menu_snapshot_for_settings(&providers, settings, menu_path)
}

fn native_menu_context_for_item(
    settings: &codexbar::settings::Settings,
    item_id: &str,
...
mod tests {
    ...
    fn about_path_snapshot_persists_only_after_success() {
        ...
        let settings = codexbar::settings::Settings::default();
        let result = persist_about_path_snapshot(&settings, Ok(()));
    ...
    fn about_path_snapshot_clears_on_failure() {
        ...
        let settings = codexbar::settings::Settings::default();
        let result = persist_about_path_snapshot(&settings, Err("boom".into()));
    ...
    fn about_path_snapshot_fails_when_about_item_is_missing() {
        ...
        let settings = codexbar::settings::Settings::default();
        let result = persist_about_path_snapshot_for_item(&settings, "missing-about", Ok(()));
