...
export default function ProviderGrid({
  ...
  const gridPercent = (provider: ProviderPresentationGroup) => {
    const snapshot = provider.successfulSnapshots[0] ?? provider.snapshots[0];
    ...
    >
      ...
      {visibleProviders.map((p) => (
        <button
          ...
          aria-label={
            p.hasMultipleCredentials
              ? t("ApiKeyProviderCount")
                  .replace("{}", p.displayName)
                  .replace("{}", String(p.credentialCount))
              : p.displayName
          }
        ...
        >
          ...
          {p.hasMultipleCredentials ? (
            <span className="provider-grid__key-count">
              {t("ApiKeyCount").replace("{}", String(p.credentialCount))}
            </span>
          ) : !p.isAllFailed && p.snapshots.length > 0 && (
            <span
