...
export function listKimiAccounts(): Promise<KimiAccountSnapshot[]> {
  return invoke<KimiAccountSnapshot[]>("list_kimi_accounts");
}

export function refreshKimiBrowserAccounts(): Promise<KimiBrowserScanResult> {
  return invoke<KimiBrowserScanResult>("refresh_kimi_browser_accounts");
}

export function beginKimiAccountLogin(
  expectedAccountId?: string,
): Promise<KimiLoginSession> {
  return invoke<KimiLoginSession>("begin_kimi_account_login", {
    expectedAccountId: expectedAccountId ?? null,
  });
}

export function completeKimiAccountLogin(
  sessionId: string,
  expectedAccountId?: string,
): Promise<KimiLoginCompletion> {
  return invoke<KimiLoginCompletion>("complete_kimi_account_login", {
    sessionId,
    expectedAccountId: expectedAccountId ?? null,
  });
}

export function cancelKimiAccountLogin(sessionId: string): Promise<boolean> {
  return invoke<boolean>("cancel_kimi_account_login", { sessionId });
}

export function removeKimiAccount(accountId: string): Promise<void> {
  return invoke<void>("remove_kimi_account", { accountId });
}
...
export function getApiKeySecret(
  ...
  return invoke<string>("get_api_key_secret", { providerId, credentialId });
}
...
export function addApiKey(
  ...
  return invoke<ApiKeyInfoBridge[]>("add_api_key", {
    providerId,
    apiKey,
    label: label ?? null,
  });
...
export function updateApiKeyLabel(
  providerId: string,
  credentialId: string,
  label?: string,
): Promise<ApiKeyInfoBridge[]> {
  return invoke<ApiKeyInfoBridge[]>("update_api_key_label", {
    providerId,
    credentialId,
    label: label ?? null,
  });
...
export function replaceApiKeySecret(
  providerId: string,
  credentialId: string,
  apiKey: string,
): Promise<ApiKeyInfoBridge[]> {
  return invoke<ApiKeyInfoBridge[]>("replace_api_key_secret", {
    providerId,
    credentialId,
    apiKey,
  });
...
export function deleteApiKey(
  ...
  return invoke<ApiKeyInfoBridge[]>("delete_api_key", {
    providerId,
    credentialId,
  });
}
...
export function reorderApiKeys(
  ...
  return invoke<ApiKeyInfoBridge[]>("reorder_api_keys", {
    providerId,
    credentialIds,
  });
}
...
export function getRuntimeIdentity(): Promise<RuntimeIdentityBridge> {
  return invoke<RuntimeIdentityBridge>("get_runtime_identity");
}
