...
import type { KimiAccountSnapshot, ProviderUsageSnapshot } from "../types/bridge";

function kimiAccount(status: KimiAccountSnapshot["status"], usedPercent = 42): KimiAccountSnapshot {
  return {
    accountId: "account-1",
...
function provider(
  ...
  return {
    ...
    credentialDisplayLabel: credential?.label,
    credentialDisplayOrdinal: credential?.ordinal,
    primary: {
      usedPercent,
...
describe("useProviders", () => {
  ...
  for (const origin of ["initial load", "settings reload"] as const) {
    it.each([
      ["fresh quota", [kimiAccount("ok", 75)]],
      ["storage failure", [kimiAccount("storageError", 42)]],
      ["account removal", []],
    ])(`does not let a late Kimi ${origin} overwrite a newer %s event`, async (_label, latest) => {
      let resolveOld!: (accounts: KimiAccountSnapshot[]) => void;
      const oldResponse = new Promise<KimiAccountSnapshot[]>((resolve) => { resolveOld = resolve; });
      if (origin === "initial load") {
        tauriMocks.listKimiAccounts.mockReturnValueOnce(oldResponse);
      } else {
        tauriMocks.listKimiAccounts.mockResolvedValueOnce([kimiAccount("stale", 10)]);
        tauriMocks.listKimiAccounts.mockReturnValueOnce(oldResponse);
      }
      const { result } = renderHook(() => useProviders({ refreshOnMount: false }));
      await act(async () => {});
      if (origin === "settings reload") {
        await act(async () => emitProviderEvent("settings-changed", undefined));
      }
      act(() => emitProviderEvent("kimi-accounts-updated", latest));
      await act(async () => resolveOld([kimiAccount("ok", 10)]));
      expect(result.current.kimiAccounts).toEqual(latest);
    });
  }

  it("keeps a completed Kimi settings reload when the initial load finishes later", async () => {
    let resolveInitial!: (accounts: KimiAccountSnapshot[]) => void;
    tauriMocks.listKimiAccounts.mockReturnValueOnce(new Promise<KimiAccountSnapshot[]>((resolve) => {
      resolveInitial = resolve;
    }));
    tauriMocks.listKimiAccounts.mockResolvedValueOnce([kimiAccount("ok", 75)]);
    const { result } = renderHook(() => useProviders({ refreshOnMount: false }));
    await act(async () => {});
    await act(async () => emitProviderEvent("settings-changed", undefined));
    await act(async () => resolveInitial([kimiAccount("ok", 10)]));
    expect(result.current.kimiAccounts[0]?.usedPercent).toBe(75);
  });
  ...
  it("keeps sibling credentials and the default provider lane distinct", async () => {
    tauriMocks.getCachedProviders.mockResolvedValue([
      provider("openrouter", 10),
      provider("openrouter", 20, { id: "credential-a", label: "Work", ordinal: 1 }),
      provider("openrouter", 30, { id: "credential-b", label: "Personal", ordinal: 2 }),
    ]);

    const { result } = renderHook(() => useProviders({ refreshOnMount: false }));

    await waitFor(() => expect(result.current.providers).toHaveLength(3));
    expect(result.current.providers.map((snapshot) => snapshot.credentialId ?? null)).toEqual([
      null,
      "credential-a",
      "credential-b",
    ]);
  ...
  it("replaces only the matching credential when a live update arrives", async () => {
    tauriMocks.getCachedProviders.mockResolvedValue([
      provider("openrouter", 20, { id: "credential-a", label: "Work", ordinal: 1 }),
      provider("openrouter", 30, { id: "credential-b", label: "Personal", ordinal: 2 }),
    ]);
    ...
    act(() => {
      emitProviderEvent(
        "provider-updated",
        provider("openrouter", 55, { id: "credential-a", label: "Work", ordinal: 1 }),
      );
      emitProviderEvent("refresh-complete", {
        providerCount: 1,
        errorCount: 0,
        degradedCredentialCount: 0,
      });
    });

    expect(result.current.providers).toHaveLength(2);
    expect(
      result.current.providers.map((snapshot) => [
  ...
  it("preserves credential siblings across a settings cache reload", async () => {
    tauriMocks.getCachedProviders
      .mockResolvedValueOnce([
        provider("openrouter", 10, { id: "credential-a", label: "Work", ordinal: 1 }),
        provider("openrouter", 20, { id: "credential-b", label: "Personal", ordinal: 2 }),
      ])
      .mockResolvedValueOnce([
        provider("openrouter", 30, { id: "credential-a", label: "Work", ordinal: 1 }),
        provider("openrouter", 40, { id: "credential-b", label: "Personal", ordinal: 2 }),
      ]);
    ...
    await waitFor(() =>
      expect(result.current.providers.map((snapshot) => snapshot.primary.usedPercent)).toEqual([
        30,
        40,
      ]),
    );
  ...
  it("drops credentials removed by a settings cache reload", async () => {
    tauriMocks.getCachedProviders
      .mockResolvedValueOnce([
        provider("openrouter", 10, { id: "credential-a", label: "Work", ordinal: 1 }),
        provider("openrouter", 20, { id: "credential-b", label: "Personal", ordinal: 2 }),
    ...
    await waitFor(() => {
      ...
      expect(result.current.providers[0]?.credentialId).toBe("credential-b");
      expect(result.current.providers[0]?.primary.usedPercent).toBe(30);
    });
  });
  ...
  it("keeps provider refresh state until refresh-complete after a sibling update", async () => {
