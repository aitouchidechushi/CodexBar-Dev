...
export function selectDenseOverviewProviders(
  providers: ProviderPresentationGroup[],
  limit = 4,
): ProviderPresentationGroup[] {
  if (providers.length <= limit) return providers;
  const first = providers.slice(0, limit);
  const monthlyAccountGroup = providers.find(
    (provider) => (provider.kimiAccounts?.length ?? 0) > 0,
  );
  if (!monthlyAccountGroup || first.includes(monthlyAccountGroup)) return first;
  const selected = new Set([
    ...first.slice(0, Math.max(0, limit - 1)).map((provider) => provider.providerId),
    monthlyAccountGroup.providerId,
  ]);
  return providers.filter((provider) => selected.has(provider.providerId)).slice(0, limit);
}
...
export default function TrayPanel({ state }: { state: BootstrapState }) {
  ...
  const submitAddKey = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  const providersById = useMemo(
    () => new Map(sorted.map((group) => [group.providerId, group])),
    [sorted],
  );
  ...
  const denseTrayProviders = useMemo(() => {
    ...
    const hydrated = hydrateProviderSlots(denseProviderSlots, providersById);
    const included = new Set(hydrated.map((group) => group.providerId));
    const combined = [
      ...hydrated,
      ...sorted.filter((group) => !included.has(group.providerId)),
    ];
    ...
    return combined
      ...
      .sort((left, right) => {
        const leftOrder = orderIndex.get(left.group.providerId);
        const rightOrder = orderIndex.get(right.group.providerId);
      ...
      .map(({ group }) => group);
  }, [denseProviderSlots, expectsDenseOverview, providersById, settings.providerOrder, sorted]);
  ...
  const visibleProviders = useMemo(() => {
    if (selectedProviderId === null) {
      ...
      if (sorted.length + 1 > DENSE_OVERVIEW_THRESHOLD && !gridExpanded) {
        return selectDenseOverviewProviders(denseTrayProviders);
      }
      return sorted;
    }
  ...
  const wideColumns = useMemo(() => {
    const columns: ProviderPresentationGroup[][] = [[], []];
  ...
  const renderProviderCard = (p: ProviderPresentationGroup) => {
    const isSelected =
