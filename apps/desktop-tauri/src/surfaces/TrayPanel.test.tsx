...
describe("TrayPanel provider grid", () => {
  ...
  it("groups sibling credentials into one routed grid item and one provider card", async () => {
    const first = {
    ...
    expect(screen.getByRole("region", { name: "Work" })).toHaveTextContent("20% used");
    expect(screen.getByRole("region", { name: "Key 2" })).toHaveTextContent("90% used");
    expect(screen.getByRole("button", { name: "Codex, 2 keys" })).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Codex, 2 keys" }));
    expect(container.querySelectorAll(".menu-card")).toHaveLength(1);
    await waitFor(() => {
  ...
  it("adds a MiniMax Token Plan Key from the card action and leaves unsupported cards unchanged", async () => {
    renderTrayPanel(
      [provider("minimax", "MiniMax"), provider("claude", "Claude")],
      { enabledProviders: ["minimax", "claude"] },
      [
    ...
    const addButtons = await screen.findAllByRole("button", { name: "＋ 添加 API Key" });
    expect(addButtons).toHaveLength(1);
    fireEvent.click(addButtons[0]);

    const key = screen.getByLabelText("Token Plan Key");
    const label = screen.getByLabelText("ApiKeyLabelOptional");
    fireEvent.change(key, { target: { value: "  minimax-key  " } });
    fireEvent.change(label, { target: { value: "  Main  " } });
    fireEvent.click(screen.getByRole("button", { name: "ApiKeyAdd" }));
  ...
  it("localizes the existing duplicate-key rejection in the card dialog", async () => {
    tauriMocks.addApiKey.mockRejectedValue(
      "An identical API key already exists for this provider",
    );
    renderTrayPanel(
      [provider("kimi", "Kimi")],
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "＋ 添加 API Key" }),
    );
    fireEvent.change(screen.getByLabelText("ApiKeyNew"), {
      target: { value: "duplicate-key" },
    });
    fireEvent.click(screen.getByRole("button", { name: "ApiKeyAdd" }));

    expect(
      await screen.findByText("该服务商已存在相同的 API Key。"),
    ).toBeInTheDocument();
  });
  ...
  it.each([
    ...
    [12, false],
      ...
      const providers = [
        ...
        provider("kiro", "Kiro"),
        provider("zai", "GLM（智谱 BigModel / z.ai）"),
  ...
  it("keeps an account-only Kimi monthly quota visible in the dense overview", async () => {
    const denseCatalog = TEST_PROVIDER_CATALOG.slice(0, 36).map(
      ([id, displayName]) => ({ id, displayName, cookieDomain: null }),
    );
    const denseProviders = denseCatalog
      .filter(({ id }) => id !== "kimi")
      .map(({ id, displayName }) => provider(id, displayName));
    const providerOrder = denseCatalog.map(({ id }) => id);
    tauriMocks.listKimiAccounts.mockResolvedValue([
      {
        accountId: "account-a",
        displayName: "账号 A",
    ...
    expect(await screen.findByRole("region", { name: "账号 A" })).toBeInTheDocument();
  });
