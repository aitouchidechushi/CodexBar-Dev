...
export default function GeneralTab({
  ...
  const handleKimiMonthlyChange = useCallback((enabled: boolean) => {
    if (!enabled) {
      set({ kimiMonthlyQuotaEnabled: false });
      return;
    }
    if (settings.kimiMonthlyConsentAccepted) {
      set({ kimiMonthlyQuotaEnabled: true });
      return;
    }
    if (window.confirm(t("KimiMonthlyConsentMessage"))) {
      set({
        kimiMonthlyQuotaEnabled: true,
        kimiMonthlyConsentAccepted: true,
      });
    }
  }, [set, settings.kimiMonthlyConsentAccepted, t]);
  ...
  return (
    <>
      ...
      {mode === "general" && <section className="settings-section">
        ...
        <div className="settings-section__group">
          <Field label={t("StartAtLogin")} description={settings.startAtLoginIssue || t("StartAtLoginHelper")} leading>
            <Toggle
      ...
      {mode === "general" && <section className="settings-section">
        <h3 className="settings-section__title">{t("FloatingBallSectionTitle")}</h3>
        <div className="settings-section__group">
          <Field
            label={t("FloatingBallShow")}
            description={t("FloatingBallShowHelper")}
            leading
          >
            <Toggle
              checked={settings.floatingBallEnabled ?? false}
              ariaLabel={t("FloatingBallShow")}
              disabled={saving}
              onChange={(v) => set({ floatingBallEnabled: v })}
            />
          </Field>
        </div>
      </section>}
      ...
      {mode === "general" && <section className="settings-section">
        <h3 className="settings-section__title">{t("KimiMonthlySectionTitle")}</h3>
        <div className="settings-section__group">
          <Field
            label={t("KimiMonthlyQuotaShow")}
            description={t("KimiMonthlyQuotaShowHelper")}
            leading
          >
            <Toggle
              checked={settings.kimiMonthlyQuotaEnabled ?? false}
              ariaLabel={t("KimiMonthlyQuotaShow")}
              disabled={saving}
              onChange={handleKimiMonthlyChange}
            />
          </Field>
        </div>
      </section>}
