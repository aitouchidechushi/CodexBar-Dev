...
function renderSharePng(summary: UsageSpendSummary, title: string, keyCountTemplate: string): string {
  ...
  if (rows.length === 0) {
    ...
    rows.forEach((row, idx) => {
      ...
      const cells = [
        ...
        usageSpendRowCurrency(row),
        usageSpendRowSource(row, keyCountTemplate),
      ];
      let cx = pad;
...
export default function UsageSpendTab(_props: TabProps) {
  ...
  const onShare = useCallback(() => {
    ...
    try {
      const dataUrl = renderSharePng(summary, t("UsageSpendTitle"), t("ApiKeyCount"));
      if (!dataUrl) {
          ...
          <tbody>
            {(summary?.rows ?? []).map((row) => (
              <tr key={row.providerId}>
                ...
                <td>{usageSpendRowCurrency(row)}</td>
                <td className="usage-spend-table__source">
                  {usageSpendRowSource(row, t("ApiKeyCount"))}
                </td>
              </tr>
            ))}
...
export function usageSpendRowSource(row: UsageSpendRow, keyCountTemplate: string): string {
  return row.credentialCount && row.credentialCount > 1
    ? keyCountTemplate.replace("{}", String(row.credentialCount))
    : row.source;
}

export function usageSpendRowCurrency(row: UsageSpendRow): string {
  return row.credentialCount && row.credentialCount > 1
    ? "—"
    : row.currency || "USD";
}
