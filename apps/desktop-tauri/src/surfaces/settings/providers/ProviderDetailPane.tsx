...
export function ProviderDetailPane({
  ...
  return (
    <div className="provider-detail">
      ...
      {detail.lastError && (detail.credentialCount ?? 0) <= 1 && (
        <ProviderIssueNotice
          detail={detail}
...
export function ProviderDetailUsageState({
  detail,
  resetTimeRelative,
}: {
  ...
  const { t } = useLocale();
  const credentialCount = detail.credentialCount ?? 0;
  const failedCredentialCount = detail.failedCredentialCount ?? 0;
  if (credentialCount > 1) {
    const count = t("ApiKeyCount").replace("{}", String(credentialCount));
    const allFailed = failedCredentialCount >= credentialCount;
    const failed = t("ApiKeyFailureCount")
      .replace("{}", String(failedCredentialCount))
      .replace("{}", String(credentialCount));
    return (
      <section className="provider-detail__section provider-detail__multi-key-state" role="status">
        <strong>{count}</strong>
        {allFailed ? (
          <span>{t("ProviderStatusError")}</span>
  ...
  return (
    <UsageSection provider={detail} resetTimeRelative={resetTimeRelative} t={t} />
  );
}
