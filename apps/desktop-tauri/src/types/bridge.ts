...
export interface SettingsSnapshot {
  ...
  kimiMonthlyConsentAccepted?: boolean;
  kimiBrowserConsentAccepted?: boolean;
...
export interface ProviderUsageSnapshot {
  providerId: string;
  displayName: string;
  /** Stable app-managed credential identity. Absent for the default provider job. */
  credentialId?: string;
  /** Safe backend-provided label for an app-managed credential. */
  credentialDisplayLabel?: string;
  /** Stable backend-provided display order for an app-managed credential. */
  credentialDisplayOrdinal?: number;
  credentialGroupSize?: number;
  ...
  /** Stable provider-scoped membership key. Internal only; never render it. */
  accountGroupId?: string;
  /** Provider-supplied display name. Never use it for identity matching. */
  accountDisplayName?: string;
  sourceLabel: string;
  updatedAt: string;
  error: string | null;
  /** The latest refresh failed transiently; quota values are from the last successful fetch. */
  refreshError?: string;
...
export interface KimiAccountSnapshot {
  accountId: string;
  displayName: string;
  usedPercent: number | null;
  resetsAt: string | null;
  updatedAt: string | null;
  status: "loading" | "ok" | "stale" | "storageError" | "refreshFailed" | "loginRequired";
  matchedCredentialIds: string[];
  sourceLabels?: string[];
}

export interface KimiBrowserScanResult {
  accounts: KimiAccountSnapshot[];
  discovered: number;
  refreshed: number;
  failed: number;
}

export interface KimiLoginSession {
  sessionId: string;
}

export type KimiLoginCompletion =
  | { status: "completed"; snapshot: KimiAccountSnapshot }
  | { status: "pending"; snapshot: null };

export interface KimiLoginCompletionEvent {
  sessionId: string;
  status: "completed" | "failed" | "cancelled";
  snapshot: KimiAccountSnapshot | null;
  error: string | null;
}
...
export interface ApiKeyInfoBridge {
  ...
  savedAt: string;
  active?: boolean;
  inactiveReason?: "over-limit-migration" | "user-disabled" | null;
}
...
export interface BuildIdentityBridge {
  productName: string;
...
export interface RuntimeIdentityBridge {
  build: BuildIdentityBridge;
