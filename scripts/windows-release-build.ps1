...
function Get-PackageManifestSha256 {
    param([string]$Root)
    ...
    try {
        foreach ($relative in @(
            ...
            $name = [System.Text.Encoding]::UTF8.GetBytes($relative)
            $bytes = [System.IO.File]::ReadAllBytes((Join-Path $Root $relative))
            foreach ($part in @($name, [byte[]]@(0), $bytes, [byte[]]@(0))) {
                [void]$hasher.TransformBlock($part, 0, $part.Length, $part, 0)
            }
        }
...
try {
    ...
    $env:CODEXBAR_BUILD_CHANNEL = 'stable'
    $env:CODEXBAR_RELEASE_COMMIT = $commit
    $env:CODEXBAR_BUILD_TIMESTAMP = [DateTime]::UtcNow.ToString('o')
    $rustVersion = (& rustc --version | Out-String).Trim()
    if ($LASTEXITCODE -ne 0 -or -not $rustVersion) { throw 'Cannot determine the release Rust toolchain' }
    $env:CODEXBAR_TOOLCHAINS_JSON = @{ rustc = $rustVersion } | ConvertTo-Json -Compress
    ...
    if ($null -eq $process.ExitCode) {
        throw "pnpm tauri build completed without a reliable exit code; an existing binary does not prove this build succeeded"
    }
    ...
    $manifestSha256 = Get-PackageManifestSha256 -Root $SourceDir
    ...
    try {
        Invoke-Native $iscc @(
            "/Qp",
            "/DAppVersion=$version",
            "/DExpectedCommit=$commit",
            "/DExpectedManifestSha256=$manifestSha256",
    ...
    if ($UploadRelease) {
        ...
        Invoke-Native $gh.Source (@("release", "upload", $UploadRelease) + $assetPaths)
    }
