param(
    ...
    [string]$InstallDir = "$env:LOCALAPPDATA\Programs\CodexBar\v2",
...
foreach ($name in @("codexbar", "codexbar-desktop", "codexbar-desktop-tauri")) {
    if (Get-Process -Name $name -ErrorAction SilentlyContinue) {
        throw 'CodexBar is running. Use a disposable test environment; this script will not stop user processes.'
    }
}
$canonicalDir = [IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA 'Programs\CodexBar\v2'))
if ([IO.Path]::GetFullPath($InstallDir).TrimEnd('\') -ine $canonicalDir.TrimEnd('\')) {
    throw 'The smoke test must target the canonical v2 installation directory.'
}
if ((Test-Path -LiteralPath $canonicalDir) -or (Test-Path 'HKCU:\Software\CodexBar\InstallV2')) {
    throw 'An existing v2 installation was found. Use a clean disposable test environment.'
}
...
$uninstallKeys = @(
    "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\{6F73B2E3-8E5B-4A2D-9E7B-0C46A0B2F001}_is1"
)
$uninstallEntry = $null
...
if ($null -eq $uninstallEntry) {
    throw "Missing CodexBar v2 uninstall registry entry."
}
...
if (-not $LeaveInstalled) {
    ...
    if ([IO.Path]::GetFullPath($uninstaller) -ine (Join-Path $canonicalDir 'unins000.exe')) {
        throw 'The uninstall command does not belong to the tested v2 installation.'
    }
