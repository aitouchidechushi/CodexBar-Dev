...
pub(super) struct RawSettings {
    ...
    /// Repository loads inspect this map so a legacy field cannot disappear
    /// without an auditable warning. Ordinary `Settings` deserialization keeps
    /// its historical forward-compatible behavior.
    #[serde(flatten)]
    unknown_fields: BTreeMap<String, Value>,
}

impl RawSettings {
    pub(super) fn from_value_with_unknown_fields(
        value: Value,
    ) -> Result<(Settings, Vec<String>), serde_json::Error> {
        let raw = serde_json::from_value::<Self>(value)?;
        let unknown_fields = raw.unknown_fields.keys().cloned().collect();
        Ok((raw.into(), unknown_fields))
    }
}
...
impl Default for RawSettings {
    fn default() -> Self {
        ...
        Self {
            ...
            floating_ball_enabled: s.floating_ball_enabled,
            kimi_monthly_quota_enabled: s.kimi_monthly_quota_enabled,
            kimi_monthly_consent_accepted: s.kimi_monthly_consent_accepted,
            kimi_browser_consent_accepted: s.kimi_browser_consent_accepted,
...
impl From<RawSettings> for Settings {
    fn from(raw: RawSettings) -> Self {
        ...
        Settings {
            ...
            floating_ball_enabled: raw.floating_ball_enabled,
            kimi_monthly_quota_enabled: raw.kimi_monthly_quota_enabled,
            kimi_monthly_consent_accepted: raw.kimi_monthly_consent_accepted,
            kimi_browser_consent_accepted: raw.kimi_browser_consent_accepted,
