...
#[test]
fn floating_ball_defaults_off_and_loads_legacy_settings() {
    let defaults = Settings::default();
    assert!(!defaults.floating_ball_enabled);

    let legacy: Settings = serde_json::from_str("{}").expect("解析旧设置");
    assert!(!legacy.floating_ball_enabled);
}

#[test]
fn kimi_monthly_quota_defaults_off_and_preserves_consent() {
    let defaults = Settings::default();
    assert!(!defaults.kimi_monthly_quota_enabled);
    assert!(!defaults.kimi_monthly_consent_accepted);

    let legacy: Settings = serde_json::from_str("{}").expect("parse legacy settings");
    assert!(!legacy.kimi_monthly_quota_enabled);
    assert!(!legacy.kimi_monthly_consent_accepted);
    ...
    let roundtrip: Settings = serde_json::from_str(&serde_json::to_string(&configured).unwrap())
        .expect("roundtrip settings");
    assert!(roundtrip.kimi_monthly_quota_enabled);
    assert!(roundtrip.kimi_monthly_consent_accepted);
}

#[test]
fn kimi_browser_consent_defaults_false_and_roundtrips_true() {
    let defaults = Settings::default();
    assert!(!defaults.kimi_browser_consent_accepted);
    let encoded = serde_json::to_string(&Settings {
        kimi_browser_consent_accepted: true,
        ..defaults
    })
    .unwrap();
    let roundtrip: Settings = serde_json::from_str(&encoded).unwrap();
    assert!(roundtrip.kimi_browser_consent_accepted);

    let legacy: Settings = serde_json::from_str("{}").unwrap();
    assert!(!legacy.kimi_browser_consent_accepted);
}
...
fn float_bar_settings_round_trip_through_raw() {
    ...
    assert!(back.floating_ball_enabled);
...
#[test]
fn api_key_display_contains_label_but_no_unicode_secret_material() {
    let mut keys = ApiKeys::default();
    keys.set("openrouter", "🔑🔒漢字abcdefgh🔐", Some("unicode"));

    let display = keys.get_all_for_display();
    let json = serde_json::to_string(&display).expect("serialize display info");

    assert_eq!(display.len(), 1);
    assert_eq!(display[0].label, "unicode");
    assert!(!json.contains("🔑"));
    assert!(!json.contains("abcdefgh"));
}

#[test]
fn test_app_managed_api_key_catalog_excludes_cookie_only_providers() {
    let providers = get_app_managed_api_key_providers();

    assert!(
        providers
            .iter()
            .any(|provider| provider.id == ProviderId::Ollama)
    );
    assert!(
        providers
            .iter()
            .any(|provider| provider.id == ProviderId::MiniMax)
    );
    assert!(
        !providers
            .iter()
            .any(|provider| provider.id == ProviderId::Alibaba)
    );
    assert!(
        !providers
            .iter()
            .any(|provider| provider.id == ProviderId::Grok)
    );
}
...
#[test]
fn legacy_settings_save_cannot_write_after_the_v2_store_exists() {
    let directory = tempfile::tempdir().unwrap();
    let legacy = directory.path().join("settings.json");
    let v2 = directory.path().join("v2").join("settings.json");
    std::fs::create_dir_all(v2.parent().unwrap()).unwrap();
    let v2_bytes = b"fixture-v2-settings-must-not-change";
    std::fs::write(&v2, v2_bytes).unwrap();

    let error = Settings::default()
        .save_legacy_to_path(&legacy, Some(&v2))
        .unwrap_err();

    assert!(error.to_string().contains("StorageV2"));
    assert!(!legacy.exists());
    assert_eq!(std::fs::read(v2).unwrap(), v2_bytes);
}
...
#[test]
fn test_settings_missing_theme_defaults_to_dark() {
    // Legacy settings JSON without the theme field should use the app's dark default.
    let legacy_json = r#"{
            "enabled_providers": ["claude", "codex"],
            "refresh_interval_secs": 300,
            "ui_language": "english"
...
#[test]
fn test_settings_legacy_auto_theme_uses_dark_default() {
    let legacy_json = r#"{
            "theme": "auto"
        }"#;
