...
//!
//! Loads rules through the versioned hook store and runs matching binaries
//! with a narrow env whitelist + JSON stdin payload.

use super::hooks_store::HooksConfigRepository;
use crate::storage::{LoadState, StoreKind};
...
fn load_config_for_dispatch(repository: &HooksConfigRepository) -> Option<HooksConfig> {
    match repository.load() {
        Ok(LoadState::Loaded(config)) => Some(config),
        Ok(LoadState::Missing) => None,
        Ok(state) => {
            tracing::warn!(
                store = %StoreKind::Hooks,
                state = ?state.kind(),
                "hook dispatch skipped because hook storage requires recovery"
            );
            None
        }
        Err(error) => {
            tracing::warn!(
                store = %StoreKind::Hooks,
                error = %error,
                "hook dispatch skipped because hook storage is unavailable"
            );
            None
        }
    }
...
impl HookRunner {
    ...
    pub fn dispatch_if_enabled(event: HookEvent, hooks_enabled: bool) {
        ...
        let repository = match HooksConfigRepository::discover() {
        ...
        let Some(config) = load_config_for_dispatch(&repository) else {
            return;
        };
        if !config.enabled {
            return;
...
pub fn emit_quota_threshold_hooks(
    ...
    let crossed_low_threshold = (previous < critical_threshold && used >= critical_threshold)
        || (previous < high_threshold && used >= high_threshold);
    if crossed_low_threshold && used < 100.0 {
...
mod tests {
    ...
    #[test]
    fn dispatch_does_not_treat_a_corrupt_hook_store_as_an_empty_configuration() {
        let root = std::env::temp_dir().join(format!(
            "codexbar-hook-dispatch-corrupt-{}",
            uuid::Uuid::new_v4()
        ));
        let paths = StoragePaths::from_roots(root.join("roaming"), root.join("local"));
        let repository = HooksConfigRepository::with_protection(
            paths.clone(),
            UserScopeId::from_stable_identifier(b"hook-dispatch-test-user"),
            Protection::Plaintext,
            Arc::new(PlatformSecretProtector),
        );
        let original = b"broken-hooks-config";
        fs::create_dir_all(paths.hooks().parent().unwrap()).unwrap();
        fs::write(paths.hooks(), original).unwrap();

        assert!(load_config_for_dispatch(&repository).is_none());
        assert_eq!(fs::read(paths.hooks()).unwrap(), original);
        fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn dispatch_loader_uses_a_later_manual_public_hook_edit() {
        let root = std::env::temp_dir().join(format!(
            "codexbar-hook-dispatch-public-edit-{}",
            uuid::Uuid::new_v4()
        ));
        let paths = StoragePaths::from_roots(root.join("roaming"), root.join("local"));
        let repository = HooksConfigRepository::with_protection(
            paths.clone(),
            UserScopeId::from_stable_identifier(b"hook-dispatch-public-edit-test-user"),
            Protection::Plaintext,
            Arc::new(PlatformSecretProtector),
        );
        #[cfg(windows)]
        ...
        fs::create_dir_all(paths.hooks().parent().unwrap()).unwrap();
        fs::write(paths.hooks(), serde_json::to_vec(&original).unwrap()).unwrap();
        let event = HookEvent::new(HookEventType::QuotaLow, "codex").with_used_percent(90.0);

        assert!(
            load_config_for_dispatch(&repository)
                .unwrap()
