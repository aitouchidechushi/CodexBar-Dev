...
impl std::fmt::Debug for HttpProxySettings {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("HttpProxySettings")
            .field("enabled", &self.enabled)
            .field("url_set", &!self.url.is_empty())
            .field("username_set", &!self.username.is_empty())
...
impl HttpProxySettings {
    ...
    /// Copy the proxy fields from an already verified settings snapshot.
...
fn app_proxy_settings() -> &'static RwLock<HttpProxySettings> {
    static SETTINGS: LazyLock<RwLock<HttpProxySettings>> =
        LazyLock::new(|| RwLock::new(HttpProxySettings::default()));
    &SETTINGS
}

/// Install a verified process-wide proxy snapshot.
///
/// Provider constructors do not own the settings repository, so the desktop
/// and CLI process boundaries call this after they have handled every
/// non-readable settings state. This function never reads from disk.
pub fn configure_app_proxy(settings: &Settings) {
    *app_proxy_settings()
        .write()
        .unwrap_or_else(|poisoned| poisoned.into_inner()) =
        HttpProxySettings::from_settings(settings);
}
...
pub fn apply_app_proxy(builder: ClientBuilder) -> ClientBuilder {
    let settings = app_proxy_settings()
        .read()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .clone();
    apply_proxy_to_builder(builder, &settings)
}
...
mod tests {
    ...
    #[test]
    fn proxy_snapshot_is_built_from_explicit_settings() {
        let settings = Settings {
            http_proxy_enabled: true,
            http_proxy_url: "http://proxy.local:8080".to_string(),
            http_proxy_username: "alice".to_string(),
            http_proxy_password: "secret".to_string(),
            ..Settings::default()
        };

        let snapshot = HttpProxySettings::from_settings(&settings);

        assert!(snapshot.enabled);
        assert_eq!(snapshot.url, "http://proxy.local:8080");
        assert_eq!(snapshot.username, "alice");
        assert_eq!(snapshot.password, "secret");
    }

    #[test]
    fn proxy_debug_output_never_contains_the_password() {
        let snapshot = HttpProxySettings::from_parts(
            true,
            "http://url-user:url-secret@proxy.local:8080",
            "alice",
            "fixture-proxy-secret",
        );

        let rendered = format!("{snapshot:?}");
        assert!(!rendered.contains("fixture-proxy-secret"));
        assert!(!rendered.contains("url-secret"));
        assert!(rendered.contains("password_set"));
    }
