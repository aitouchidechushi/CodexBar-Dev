...
pub struct UsageSnapshot {
    ...
    /// Stable provider-scoped account membership key. Never intended for display.
    #[serde(default, skip_serializing)]
    pub account_group_id: Option<String>,

    /// Provider-supplied account display name. Never used for identity matching.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl UsageSnapshot {
    ...
    /// Builder pattern: set the provider-scoped stable account membership key.
    pub fn with_account_group_id(mut self, account_group_id: impl Into<String>) -> Self {
        self.account_group_id = Some(account_group_id.into());
        self
    }

    /// Builder pattern: set the provider-supplied account display name.
    pub fn with_account_display_name(mut self, account_display_name: impl Into<String>) -> Self {
        self.account_display_name = Some(account_display_name.into());
        self
    }
...
mod tests {
    ...
    #[test]
    fn account_group_id_remains_internal_while_display_name_is_serialized() {
        let anonymous = UsageSnapshot::new(RateWindow::new(10.0));
        let anonymous_json = serde_json::to_value(&anonymous).unwrap();

        assert!(anonymous_json.get("account_group_id").is_none());
        assert!(anonymous_json.get("account_display_name").is_none());

        let identified = UsageSnapshot::new(RateWindow::new(20.0))
            .with_account_group_id("factory:user-42")
            .with_account_display_name("Ada");
        let identified_json = serde_json::to_value(&identified).unwrap();

        assert_eq!(
            identified.account_group_id.as_deref(),
            Some("factory:user-42")
        );
        assert_eq!(identified.account_display_name.as_deref(), Some("Ada"));
        assert!(identified_json.get("account_group_id").is_none());
        assert_eq!(identified_json["account_display_name"], "Ada");
    }
}
