...
pub async fn check_for_updates_with_channel(channel: UpdateChannel) -> Option<UpdateInfo> {
    let release_url = release_url(channel)?;
    let client = update_client()?;
    let response = client.get(release_url).send().await.ok()?;
...
fn release_url(channel: UpdateChannel) -> Option<String> {
    let repository = GITHUB_REPO?;
    Some(match channel {
        UpdateChannel::Beta => format!("https://api.github.com/repos/{repository}/releases"),
...
/// Apply a downloaded update by spawning the installer.
///
/// This function will:
/// 1. Spawn the installer executable
///
/// The application shell remains responsible for requesting its normal exit
/// after this function returns successfully.
pub fn apply_update(installer_path: &Path) -> Result<(), String> {
    apply_update_with(installer_path, |installer_path| {
...
fn apply_update_with(
    installer_path: &Path,
    launch_installer: impl FnOnce(&Path) -> Result<(), String>,
) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    fn update_source_is_disabled_until_a_repository_is_configured() {
        assert!(release_url(UpdateChannel::Stable).is_none());
        assert!(release_url(UpdateChannel::Beta).is_none());
    }
    ...
    #[test]
    fn apply_update_returns_normally_after_launch_succeeds() {
        let temp = tempfile::tempdir().expect("temp dir");
        let path = temp.path().join("CodexBar-1.2.3-Setup.exe");
        std::fs::write(&path, b"installer").expect("write installer");
        let launched = std::cell::Cell::new(false);

        let result = apply_update_with(&path, |installer| {
            assert_eq!(installer, path.as_path());
            launched.set(true);
            Ok(())
        });
    ...
    #[test]
    fn apply_update_rejects_non_installer_without_launching() {
        let temp = tempfile::tempdir().expect("temp dir");
        let path = temp.path().join("codexbar.exe");
        std::fs::write(&path, b"portable").expect("write portable");
        let launches = std::cell::Cell::new(0);

        let result = apply_update_with(&path, |_| {
