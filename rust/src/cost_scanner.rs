...
impl CostScanner {
    ...
    pub fn new(days: u32, settings: &Settings) -> Self {
        Self {
            days,
            codex_custom_sessions_dirs: settings.codex_custom_sessions_dirs.clone(),
        }
    }
    ...
    fn get_codex_sessions_dirs(&self) -> Vec<PathBuf> {
        ...
        codex_sessions_dir_candidates(
            dirs::home_dir(),
            codex_home,
            &self.codex_custom_sessions_dirs,
            &default_wsl_roots(),
        )
    }
...
#[allow(dead_code)]
pub fn has_cost_usage_sources(settings: &Settings) -> bool {
    let scanner = CostScanner::new(1, settings);
    scanner
...
mod tests {
    ...
    #[test]
    fn scanner_uses_the_explicit_settings_snapshot_for_custom_session_dirs() {
        let custom = std::env::temp_dir().join(format!(
            "codexbar-explicit-cost-settings-{}",
            uuid::Uuid::new_v4()
        ));
        let settings = Settings {
            codex_custom_sessions_dirs: vec![custom.display().to_string()],
            ..Settings::default()
        };

        let scanner = CostScanner::new(30, &settings);

        assert!(
            scanner
                .get_codex_sessions_dirs()
                .contains(&custom.join("sessions"))
        );
    }
    ...
    fn parses_current_codex_payload_token_count_events() {
        ...
        let scanner = CostScanner::new(30, &Settings::default());
        let mut summary = CostSummary::default();
