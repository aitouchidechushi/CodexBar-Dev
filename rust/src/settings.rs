...
pub struct Settings {
    ...
    /// Show the independent desktop floating ball.
    #[serde(default)]
    pub floating_ball_enabled: bool,

    /// Show Kimi account-level monthly quota after explicit consent.
    #[serde(default)]
    pub kimi_monthly_quota_enabled: bool,

    /// Remember that the Kimi account-session explanation was accepted.
    #[serde(default)]
    pub kimi_monthly_consent_accepted: bool,

    /// Allow read-only discovery of existing Edge and Chrome Kimi sessions.
    #[serde(default)]
    pub kimi_browser_consent_accepted: bool,
...
impl Default for Settings {
    fn default() -> Self {
        ...
        Self {
            ...
            kimi_monthly_quota_enabled: false,
            kimi_monthly_consent_accepted: false,
            ...
            theme: ThemePreference::default(), // Dark unless explicitly configured otherwise
...
impl Settings {
    #[cfg(test)]
    fn save_legacy_to_path(&self, path: &Path, v2_path: Option<&Path>) -> anyhow::Result<()> {
        if v2_path.is_some_and(Path::exists) {
            anyhow::bail!("settings are managed by StorageV2; legacy direct save is disabled");
        }
    ...
    pub fn apply_start_at_login_registry(enabled: bool) -> anyhow::Result<()> {
        let registration = crate::install_ownership::verify_running_install()?;
        let mut registry = crate::install_ownership::WindowsRunValueStore::open_current_user()?;
        crate::install_ownership::set_start_at_login(&registration, enabled, &mut registry)?;
        Ok(())
    }
    ...
    pub fn is_start_at_login_enabled() -> bool {
        crate::install_ownership::current_start_at_login_enabled().unwrap_or(false)
    }
