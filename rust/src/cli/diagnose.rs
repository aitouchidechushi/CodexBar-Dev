...
use crate::settings::{
    ApiKeys, ManualCookieRepository, ManualCookies, Settings, SettingsRepository,
};
...
pub async fn run(args: DiagnoseArgs) -> anyhow::Result<()> {
    ...
    run_with_settings(args, &settings).await
}
...
async fn run_with_settings(args: DiagnoseArgs, settings: &Settings) -> anyhow::Result<()> {
    let api_keys = ApiKeys::load()?.into_writable_for(StoreKind::ApiKeys)?;
    let manual_cookies = ManualCookieRepository::discover()?
        .load()?
...
mod tests {
    ...
    #[tokio::test]
    async fn corrupt_settings_stop_diagnostics_before_provider_work() {
