...
use crate::core::{
    ...
    TokenAccountRepository, TokenAccountSupport, UsagePace, UsageSnapshot, instantiate_provider,
};
...
fn resolve_cli_api_key(
    ...
    if TokenAccountSupport::is_supported(provider_id) {
        let data = TokenAccountRepository::discover()?.load_provider(provider_id)?;
        if data.accounts.is_empty() {
            return Ok(ApiKeys::load()?
