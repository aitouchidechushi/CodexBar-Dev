...
use crate::core::{
    ProviderAccountData, ProviderId, TokenAccount, TokenAccountMutationError,
    TokenAccountRepository, TokenAccountSupport,
};
...
async fn list_accounts(provider_name: &str) -> anyhow::Result<()> {
    ...
    let repository = TokenAccountRepository::discover()?;
    let data = repository.load_provider(provider)?;
...
async fn add_account(provider_name: &str, label: &str, token: &str) -> anyhow::Result<()> {
    ...
    let repository = TokenAccountRepository::discover()?;
    repository.mutate_provider(provider, |data| {
        if data
            .accounts
            .iter()
            .any(|account| account.label.eq_ignore_ascii_case(label))
        {
            return Err(TokenAccountMutationError::Duplicate);
        }
...
async fn remove_account(provider_name: &str, account_ref: &str) -> anyhow::Result<()> {
    ...
    let repository = TokenAccountRepository::discover()?;
    let label = repository.mutate_provider(provider, |data| {
        let account =
            find_account_optional(data, account_ref).ok_or(TokenAccountMutationError::NotFound)?;
...
async fn switch_account(provider_name: &str, account_ref: &str) -> anyhow::Result<()> {
    ...
    let label = repository.mutate_provider(provider, |data| {
        ...
        let label = account.label.clone();
        let id = account.id;
        if !data.set_active_by_id(id) {
            return Err(TokenAccountMutationError::NotFound);
        }
        Ok(label)
    })?;
...
fn find_account_optional<'a>(
    data: &'a ProviderAccountData,
    account_ref: &str,
) -> Option<&'a TokenAccount> {
    if let Ok(index) = account_ref.parse::<usize>()
        && index > 0
        && index <= data.accounts.len()
    {
        return data.accounts.get(index - 1);
    }
    data.accounts
        .iter()
        .find(|account| account.label.eq_ignore_ascii_case(account_ref))
        .or_else(|| {
            data.accounts
                .iter()
                .find(|account| account.id.to_string().starts_with(account_ref))
        })
}
