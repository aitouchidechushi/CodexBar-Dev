...
pub async fn run(args: ServeArgs) -> anyhow::Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(error) = handle_client(stream, &config, &settings).await {
...
async fn handle_client(
    ...
    let response = match parse_request(&request) {
        Ok(request) => route_request(&request, config, settings).await,
...
async fn route_request(
    ...
    match request.path.as_str() {
        ...
        "/cost" => {
            ...
            cost_response(request.query.get("provider").map(String::as_str), settings).await
        }
