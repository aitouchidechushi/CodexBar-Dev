...
#[test]
fn tray_tooltip_uses_the_explicit_language_argument() {
    let english = tray_tooltip(Language::English, "Codex", 25.0, 50.0);
    let chinese = tray_tooltip(Language::Chinese, "Codex", 25.0, 50.0);

    assert_ne!(english, chinese);
    assert!(english.contains("Session 25%"));
    assert!(chinese.contains("会话 25%"));
}
...
#[test]
fn test_multi_key_strings_exist_in_every_language_with_safe_placeholders() {
    let keys = [
        LocaleKey::ApiKeyCount,
        LocaleKey::ApiKeyProviderCount,
        LocaleKey::ApiKeyFailureCount,
        LocaleKey::ApiKeyFailureDetails,
        LocaleKey::ApiKeyEditLabel,
        LocaleKey::ApiKeyEditLabelAction,
        LocaleKey::ApiKeyReplaceSecret,
        LocaleKey::ApiKeyReplaceSecretAction,
        LocaleKey::ApiKeyShowSecret,
        LocaleKey::ApiKeyHideSecret,
        LocaleKey::ApiKeyDelete,
        LocaleKey::ApiKeyDeleteAction,
        LocaleKey::ApiKeyDeleteConfirmation,
        LocaleKey::ApiKeyNew,
        LocaleKey::ApiKeyNewLabelOptional,
        LocaleKey::ApiKeyAdd,
    ];
    ...
    for lang in Language::all() {
        for key in keys {
            ...
            assert!(!text.trim().is_empty(), "missing {key:?} in {lang:?}");
            assert_ne!(text, key.name(), "untranslated {key:?} in {lang:?}");
        }
        assert_eq!(
            format_locale(*lang, LocaleKey::ApiKeyProviderCount, &["Provider", "2"])
                .matches("{}")
                .count(),
            0,
        );
        assert_eq!(
            format_locale(*lang, LocaleKey::ApiKeyFailureCount, &["1", "2"])
                .matches("{}")
                .count(),
            0,
        );
