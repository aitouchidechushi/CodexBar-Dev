...
mod tests {
    ...
    #[tokio::test]
    async fn only_thirty_day_window_is_marked_as_a_hard_month() {
        ...
        let seven_day = result.usage.secondary.as_ref().expect("7-day window");
        assert_eq!(seven_day.used_percent, 50.0);
        assert_eq!(seven_day.window_minutes, None);
        let thirty_day = result.usage.tertiary.as_ref().expect("30-day window");
        assert_eq!(thirty_day.used_percent, 50.0);
        assert_eq!(thirty_day.window_minutes, Some(43_200));
    }

    #[tokio::test]
    async fn zero_thirty_day_quota_is_not_marked_as_a_hard_month() {
        let mut server = mockito::Server::new_async().await;
        let mock = server
            .mock("GET", "/maas/coding/usage")
            .with_status(200)
            ...
            .with_body(
                r#"{
                    "5_hour": {"quota": 5000, "used": 2500, "remain": 2500},
                    "7_day": {"quota": 30000, "used": 15000, "remain": 15000},
                    "30_day": {"quota": 0, "used": 0, "remain": 0}
                }"#,
            )
            .create_async()
            .await;
        ...
        assert_eq!(seven_day.window_minutes, None);
        let thirty_day = result.usage.tertiary.as_ref().expect("30-day window");
        assert_eq!(thirty_day.used_percent, 0.0);
        assert_eq!(thirty_day.window_minutes, None);
    }
}
        ...
        match ctx.source_mode {
            SourceMode::Auto | SourceMode::Web => {
                ...
                let tertiary = RateWindow::with_details(
                    usage.thirty_day_percentage(),
                    (usage.thirty_day.quota > 0).then_some(43_200),
                    None,
                    None,
                );
