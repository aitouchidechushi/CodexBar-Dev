...
impl OllamaProvider {
    ...
    fn resolve_api_key(ctx: &FetchContext) -> Result<Option<String>, ProviderError> {
        if let Some(key) = ctx
            .api_key
            .as_deref()
        ...
        if let Some(key) = ["OLLAMA_API_KEY", "OLLAMA_KEY"].iter().find_map(|name| {
            std::env::var(name)
                .ok()
        ...
        let api_keys = ApiKeys::load()
            ...
            .map_err(|error| {
                ProviderError::Other(format!("API key storage unavailable: {error}"))
            })?;
        Ok(api_keys
            .get("ollama")
            .and_then(|key| clean_secret(Some(key))))
    ...
    fn has_api_key(ctx: &FetchContext) -> Result<bool, ProviderError> {
        Ok(Self::resolve_api_key(ctx)?.is_some())
    }
...
enum OllamaFetchRoute {
    Auto,
    Web,
    Api,
}

fn ollama_fetch_route(source_mode: SourceMode) -> OllamaFetchRoute {
    match source_mode {
        SourceMode::Auto => OllamaFetchRoute::Auto,
        SourceMode::Web => OllamaFetchRoute::Web,
        SourceMode::OAuth | SourceMode::Cli => OllamaFetchRoute::Api,
    }
}
...
impl Provider for OllamaProvider {
    ...
    async fn fetch_usage(&self, ctx: &FetchContext) -> Result<ProviderFetchResult, ProviderError> {
        ...
        match ollama_fetch_route(ctx.source_mode) {
            OllamaFetchRoute::Auto => {
            ...
            OllamaFetchRoute::Web => {
                let usage = self.fetch_usage_web(ctx).await?;
                Ok(ProviderFetchResult::new(usage, "web"))
            }
            OllamaFetchRoute::Api => {
...
mod credential_source_tests {
    ...
    #[test]
    fn explicit_credential_source_routes_to_api_without_web_fallback() {
        assert_eq!(ollama_fetch_route(SourceMode::OAuth), OllamaFetchRoute::Api);
    }
}
