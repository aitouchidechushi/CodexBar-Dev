...
impl NanoGPTProvider {
    ...
    fn usage_snapshot_from_response(usage: UsageResponse) -> Result<UsageSnapshot, ProviderError> {
        ...
        let monthly_window = usage
            .monthly
            .as_ref()
            .map(|monthly| Self::window_from_metric(monthly, usage.limits.monthly, 30 * 24 * 60));
        let daily_window = usage.daily.as_ref().map(|daily| {
            Self::window_from_metric(daily, usage.limits.daily.unwrap_or_default(), 24 * 60)
        });
    ...
    fn window_from_metric(metric: &UsageMetric, limit: f64, window_minutes: u32) -> RateWindow {
        let mut window = RateWindow::new((metric.percent_used * 100.0).clamp(0.0, 100.0));
        window.window_minutes = Some(window_minutes);
...
mod tests {
    ...
    fn parses_documented_daily_and_monthly_usage() {
        ...
        assert_eq!(usage.primary.window_minutes, Some(24 * 60));
        assert_eq!(
