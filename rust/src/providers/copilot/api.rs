...
impl CopilotApi {
    ...
    pub async fn fetch_usage_with_token(
        ...
        let (usage, identity) = tokio::join!(
            self.fetch_quota_with_token(token, github_host),
            self.fetch_identity_with_token(token, github_host),
        );
        Ok(apply_identity_result(usage?, identity))
    }

    async fn fetch_quota_with_token(
        &self,
        token: &str,
        github_host: Option<&str>,
    ) -> Result<UsageSnapshot, ProviderError> {
...
fn apply_identity_result(
    usage: UsageSnapshot,
    identity: Result<GitHubIdentity, ProviderError>,
) -> UsageSnapshot {
    ...
    let display_name = identity
        ...
        .or_else(|| {
            let login = identity.login.trim();
            (!login.is_empty()).then_some(login)
        })
        .map(str::to_string);

    let mut usage = usage.with_account_group_id(format!("copilot:{id}"));
    if let Some(display_name) = display_name {
        usage = usage.with_account_display_name(display_name);
    }
    usage
}
...
impl UsableQuota {
    fn from_snapshot(key: &str, snapshot: QuotaSnapshot) -> Option<Self> {
        ...
        let kind_source = if id.is_empty() {
            key.as_str()
        } else {
            id.as_str()
        };
        let kind = if kind_source == "chat" {
            CopilotQuotaKind::Chat
        } else if kind_source == "completions" {
            CopilotQuotaKind::Completions
        } else if kind_source == "premium_interactions" {
    ...
    fn to_rate_window(&self, reset: Option<DateTime<Utc>>) -> RateWindow {
        ...
        let window_minutes = match self.kind {
            CopilotQuotaKind::Premium | CopilotQuotaKind::Chat | CopilotQuotaKind::Completions => {
                Some(43_200)
            }
            CopilotQuotaKind::Other => None,
        };
        RateWindow {
            used_percent,
            window_minutes,
...
mod tests {
    ...
    #[test]
    fn task_2a_copilot_identity_uses_numeric_id_and_name() {
        let usage = parse_snapshot(
            r#"{
                "quota_snapshots": {
                    "premium_interactions": {
        ...
        let identity: GitHubIdentity =
            serde_json::from_str(r#"{ "login": "octocat", "id": 583231, "name": "Mona Lisa" }"#)
                .unwrap();

        let usage = apply_identity_result(usage, Ok(identity));

        assert_eq!(usage.account_group_id.as_deref(), Some("copilot:583231"));
        assert_eq!(usage.account_display_name.as_deref(), Some("Mona Lisa"));
    }

    #[test]
    fn task_2a_copilot_identity_uses_login_when_name_is_blank() {
        let identity: GitHubIdentity =
            serde_json::from_str(r#"{ "login": "octocat", "id": 583231, "name": "  " }"#).unwrap();

        let usage = apply_identity_result(UsageSnapshot::new(RateWindow::new(12.0)), Ok(identity));

        assert_eq!(usage.account_display_name.as_deref(), Some("octocat"));
    }

    #[test]
    fn task_2a_copilot_missing_numeric_id_stays_ungrouped() {
        let identity: GitHubIdentity =
            serde_json::from_str(r#"{ "login": "octocat", "id": null, "name": "Mona" }"#).unwrap();

        let usage = apply_identity_result(UsageSnapshot::new(RateWindow::new(37.0)), Ok(identity));

        assert_eq!(usage.primary.used_percent, 37.0);
        assert!(usage.account_group_id.is_none());
        assert!(usage.account_display_name.is_none());
    }

    #[test]
    fn task_2a_copilot_identity_error_keeps_successful_quota_ungrouped() {
        let usage = apply_identity_result(
            UsageSnapshot::new(RateWindow::new(41.0)),
            Err(ProviderError::AuthRequired),
        );

        assert_eq!(usage.primary.used_percent, 41.0);
        assert!(usage.account_group_id.is_none());
        assert!(usage.account_display_name.is_none());
    }

    #[test]
    fn task_2a_copilot_proven_quota_rows_are_monthly() {
        let usage = parse_snapshot(
            r#"{
        ...
        assert_eq!(usage.primary.resets_at, parse_iso_date("2026-09-01"));
    }

    #[test]
    fn task_2a_copilot_additional_budget_is_not_monthly() {
        let usage = parse_snapshot(
            r#"{
                "quota_snapshots": {
                    "premium_interactions": {
        ...
        assert_eq!(usage.extra_rate_windows.len(), 1);
        assert_eq!(usage.extra_rate_windows[0].id, "additional-budget");
        assert_eq!(usage.extra_rate_windows[0].window.window_minutes, None);
    }

    #[test]
    fn task_2a_fix1_copilot_budget_and_balance_rows_are_not_monthly() {
        let usage = parse_snapshot(
            r#"{
                "quota_snapshots": {
                    "premium_balance": {
                        "entitlement": 100,
                        "remaining": 80,
                        "quota_id": "premium_balance"
                    },
                    "additional_premium_budget": {
                        "entitlement": 200,
                        "remaining": 150,
                        "quota_id": "additional_premium_budget"
                    },
                    "premium_interactions_balance": {
                        "entitlement": 300,
                        "remaining": 200,
                        "quota_id": "premium_interactions_balance"
                    }
                }
            }"#,
        );
        ...
        assert_eq!(usage.extra_rate_windows.len(), 3);
        for id in [
            "premium-balance",
            "additional-premium-budget",
            "premium-interactions-balance",
        ] {
            let extra = usage
                .extra_rate_windows
                .iter()
                ...
                .unwrap();
            assert_eq!(extra.window.window_minutes, None, "{id}");
        }
    }

    #[test]
    fn task_2a_fix2_nonempty_balance_quota_id_overrides_monthly_map_key() {
        let usage = parse_snapshot(
            r#"{
                "quota_snapshots": {
                    "premium_interactions": {
    ...
    #[test]
    fn task_2a_fix2_monthly_map_key_without_quota_id_remains_monthly() {
        let usage = parse_snapshot(
            r#"{
                "quota_snapshots": {
                    "premium_interactions": {
