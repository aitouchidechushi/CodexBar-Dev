...
fn coding_plan_snapshot(usage: CodingPlanResult) -> UsageSnapshot {
    ...
    for (prefix, id_prefix, scope) in [
        ("agent_", "doubao-agent", "Agent"),
        ("coding_team_", "doubao-coding-team", "Coding team"),
        ("agent_team_", "doubao-agent-team", "Agent team"),
    ] {
...
mod tests {
    ...
    fn arkcli_usage_parses_coding_and_agent_plans() {
        ...
        let agent_monthly = snap
            .extra_rate_windows
            .iter()
            .find(|w| w.id == "doubao-agent-monthly")
            .expect("agent monthly quota");
        assert_eq!(agent_monthly.title, "Agent Monthly");
        assert_eq!(agent_monthly.window.window_minutes, Some(30 * 24 * 60));
    }
