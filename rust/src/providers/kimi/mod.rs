...
mod web_session;
pub use web_session::{
    KimiWebRequestContext, KimiWebTokenPair, parse_web_request_context, refresh_web_session,
    web_refresh_token_fingerprint,
};
...
const KIMI_CURRENT_USER_URL: &str =
...
impl KimiProvider {
    ...
    fn code_api_identity_endpoint(base_url: &Url) -> Result<Url, ProviderError> {
        let base = base_url.as_str().trim_end_matches('/');
        let path = base_url.path().trim_matches('/');
        let endpoint = if path == "coding/v1" || path.ends_with("/coding/v1") {
            format!("{base}/me")
            ...
            format!("{base}/coding/v1/me")
        };
        Url::parse(&endpoint)
            .map_err(|_| ProviderError::Other("Kimi Code API identity endpoint is invalid".into()))
    }
...
mod tests {
    ...
    #[test]
    fn code_api_identity_endpoint_normalizes_base_paths() {
        let root = Url::parse("https://api.kimi.com").unwrap();
        assert_eq!(
            KimiProvider::code_api_identity_endpoint(&root)
                .unwrap()
                .as_str(),
        ...
        let versioned = Url::parse("https://proxy.example/kimi/coding/v1").unwrap();
        assert_eq!(
            KimiProvider::code_api_identity_endpoint(&versioned)
                .unwrap()
                .as_str(),
            "https://proxy.example/kimi/coding/v1/me"
        );
    }

    #[test]
    fn web_account_identity_requires_both_stable_ids() {
        let account = parse_web_account_identity(&json!({
            "user": {
                "id": "workspace-user-a",
                "globalId": "global-user-a",
                "nickname": "账号 A"
            }
        }))
        .expect("complete identity");

        assert_eq!(account.display_name.as_deref(), Some("账号 A"));
        assert_ne!(account.identity.user_id_fingerprint, "workspace-user-a");
        assert_ne!(account.identity.global_id_fingerprint, "global-user-a");
        assert!(
            parse_web_account_identity(&json!({
                "user": { "id": "workspace-user-a" }
            }))
            .is_err()
        );
    }
    ...
    #[test]
    fn code_api_identity_matches_only_when_both_fingerprints_match() {
        let account = parse_web_account_identity(&json!({
            "user": {
                "id": "workspace-user-a",
                "globalId": "global-user-a"
        ...
        .unwrap();
        let matching = parse_code_api_identity(&json!({
            "user_id": "workspace-user-a",
            "global_id": "global-user-a"
        }))
        .unwrap();
        let wrong_global = parse_code_api_identity(&json!({
            "user_id": "workspace-user-a",
            "global_id": "global-user-b"
        }))
        .unwrap();

        assert!(account.identity.strictly_matches(&matching));
        assert!(!account.identity.strictly_matches(&wrong_global));
        assert!(
            parse_code_api_identity(&json!({
                "user_id": "workspace-user-a"
            }))
            .is_err()
        );
    ...
    #[test]
    fn monthly_account_quota_parses_percentage_and_reset_time() {
        let quota = parse_monthly_account_quota(&json!({
            "subscriptionBalance": {
        ...
        assert!((quota.used_percent - 35.0).abs() < f64::EPSILON);
        assert_eq!(
            quota.resets_at.unwrap().to_rfc3339(),
            "2026-09-01T00:00:00+00:00"
        );
        assert!(
            parse_monthly_account_quota(&json!({
                "subscriptionBalance": { "amountUsedRatio": null }
            }))
            .is_err()
        );
    }
    ...
    #[tokio::test]
    async fn web_account_identity_and_quota_requests_are_polled_together() {
        let (identity_started_tx, identity_started_rx) = tokio::sync::oneshot::channel();
        let (quota_started_tx, quota_started_rx) = tokio::sync::oneshot::channel();
        let identity = async move {
            let _ = identity_started_tx.send(());
            quota_started_rx.await.unwrap();
            Ok::<_, ProviderError>("identity")
        };
        let quota = async move {
            let _ = quota_started_tx.send(());
            identity_started_rx.await.unwrap();
            Ok::<_, ProviderError>("quota")
        };

        let result = tokio::time::timeout(
            std::time::Duration::from_millis(250),
            join_web_account_parts(identity, quota),
        )
        .await
        .expect("both request futures must be polled")
        .unwrap();

        assert_eq!(result, ("identity", "quota"));
    }
}

#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct KimiIdentityFingerprint {
    pub user_id_fingerprint: String,
    pub global_id_fingerprint: String,
}

impl KimiIdentityFingerprint {
    pub fn strictly_matches(&self, other: &Self) -> bool {
        self.user_id_fingerprint == other.user_id_fingerprint
            && self.global_id_fingerprint == other.global_id_fingerprint
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct KimiAccountIdentity {
    pub identity: KimiIdentityFingerprint,
    pub display_name: Option<String>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct KimiMonthlyQuota {
    pub used_percent: f64,
    pub resets_at: Option<DateTime<Utc>>,
}

async fn join_web_account_parts<IdentityFuture, QuotaFuture, Identity, Quota>(
    identity: IdentityFuture,
    quota: QuotaFuture,
) -> Result<(Identity, Quota), ProviderError>
where
    IdentityFuture: std::future::Future<Output = Result<Identity, ProviderError>>,
    QuotaFuture: std::future::Future<Output = Result<Quota, ProviderError>>,
{
    tokio::try_join!(identity, quota)
}

pub async fn fetch_web_account(
    token: &str,
) -> Result<(KimiAccountIdentity, KimiMonthlyQuota), ProviderError> {
    ...
    let identity_request = async {
        let response =
            kimi_web_post(&client, KIMI_CURRENT_USER_URL, token, serde_json::json!({})).await?;
        ensure_kimi_auth_response(&response)?;
        let value = response
            .json::<serde_json::Value>()
            .await
            .map_err(|error| ProviderError::Parse(error.to_string()))?;
        parse_web_account_identity(&value)
    };
    let quota_request = async {
        let response = kimi_web_post(
            &client,
            KIMI_SUBSCRIPTION_STATS_URL,
            token,
            serde_json::json!({}),
        ...
        let value = response
            ...
            .await
            .map_err(|error| ProviderError::Parse(error.to_string()))?;
        parse_monthly_account_quota(&value)
    };

    join_web_account_parts(identity_request, quota_request).await
}

pub async fn fetch_code_api_identity(
    api_key: &str,
) -> Result<KimiIdentityFingerprint, ProviderError> {
    let api_key = KimiProvider::code_api_key(Some(api_key))?;
    let base_url = KimiProvider::code_api_base_url()?;
    let endpoint = KimiProvider::code_api_identity_endpoint(&base_url)?;
    let client = crate::core::credentialed_http_client_builder()
        .timeout(std::time::Duration::from_secs(30))
        .build()
        .map_err(|error| ProviderError::Other(error.to_string()))?;
    ...
    let value = response
        ...
        .await
        .map_err(|error| ProviderError::Parse(error.to_string()))?;
    parse_code_api_identity(&value)
}

fn ensure_kimi_auth_response(response: &reqwest::Response) -> Result<(), ProviderError> {
...
pub fn parse_web_account_identity(
    value: &serde_json::Value,
) -> Result<KimiAccountIdentity, ProviderError> {
    let user = value
        .get("user")
        .ok_or_else(|| ProviderError::Parse("Kimi account identity missing".into()))?;
    let user_id = required_identity_field(user, "id")?;
    let global_id = required_identity_field(user, "globalId")?;
    ...
    Ok(KimiAccountIdentity {
        identity: fingerprint_identity(user_id, global_id),
        display_name,
    })
}

pub fn parse_code_api_identity(
    value: &serde_json::Value,
) -> Result<KimiIdentityFingerprint, ProviderError> {
    let user_id = required_identity_field(value, "user_id")?;
    let global_id = required_identity_field(value, "global_id")?;
    Ok(fingerprint_identity(user_id, global_id))
}

pub fn parse_monthly_account_quota(
    value: &serde_json::Value,
) -> Result<KimiMonthlyQuota, ProviderError> {
    let response: KimiSubscriptionStatsResponse = serde_json::from_value(value.clone())
        .map_err(|error| ProviderError::Parse(error.to_string()))?;
    let balance = response
        .subscription_balance
        .ok_or_else(|| ProviderError::Parse("Kimi monthly quota missing".into()))?;
    let ratio = value_as_f64(balance.amount_used_ratio.as_ref())
        .filter(|ratio| ratio.is_finite())
        .ok_or_else(|| ProviderError::Parse("Kimi monthly quota ratio missing".into()))?;
    Ok(KimiMonthlyQuota {
        used_percent: ratio * 100.0,
        resets_at: balance.expire_time.as_ref().and_then(parse_kimi_timestamp),
    })
}
...
fn required_identity_field<'a>(
    ...
    value
        ...
        .ok_or_else(|| ProviderError::Parse(format!("Kimi identity field {field} missing")))
}

fn fingerprint_identity(user_id: &str, global_id: &str) -> KimiIdentityFingerprint {
    KimiIdentityFingerprint {
        user_id_fingerprint: fingerprint_component(user_id),
        global_id_fingerprint: fingerprint_component(global_id),
    }
}
