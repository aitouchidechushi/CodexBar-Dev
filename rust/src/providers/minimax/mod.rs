...
const UNLIMITED_WEEK_MARKER_ID: &str = "minimax-weekly-unlimited";
...
impl MiniMaxRegion {
    pub fn from_settings_value(value: Option<&str>) -> Self {
        match value.unwrap_or_default().trim().to_lowercase().as_str() {
            ...
            "global" | "io" | "international" => Self::Global,
            _ => Self::ChinaMainland,
        }
    }
    ...
    async fn fetch_token_plan(
        ...
        if !response.status().is_success() {
            return Err(ProviderError::Other(format!(
                "MiniMax Token Plan API returned status {}",
                response.status()
            )));
        }
        ...
        Ok(ProviderFetchResult::new(
            parse_token_plan_response(&json)?,
            "token-plan",
        ))
    }
...
fn token_plan_status(model: &serde_json::Value, field: &str) -> i64 {
    json_number(model.get(field)).unwrap_or_default() as i64
}

fn token_plan_total(model: &serde_json::Value, field: &str) -> f64 {
    json_number(model.get(field)).unwrap_or_default()
}

fn model_is_unavailable(model: &serde_json::Value) -> bool {
    token_plan_status(model, "current_interval_status") == 3
        && token_plan_status(model, "current_weekly_status") == 3
        && token_plan_total(model, "current_interval_total_count") == 0.0
        && token_plan_total(model, "current_weekly_total_count") == 0.0
}

fn is_text_model(model: &serde_json::Value) -> bool {
    let name = model
        .get("model_name")
    ...
    name == "general"
        ...
        || !["speech", "audio", "voice", "image", "video", "music"]
            .iter()
            .any(|kind| name.contains(kind))
}

fn select_token_plan_model(models: &[serde_json::Value]) -> Option<&serde_json::Value> {
    let available = |model: &&serde_json::Value| !model_is_unavailable(model);
    models
        .iter()
        .filter(available)
        .find(|model| {
            model
                .get("model_name")
                .and_then(serde_json::Value::as_str)
                .is_some_and(|name| name.eq_ignore_ascii_case("general"))
        })
        .or_else(|| {
            models.iter().filter(available).find(|model| {
                model
                    .get("model_name")
                    .and_then(serde_json::Value::as_str)
                    .is_some_and(|name| name.to_ascii_lowercase().contains("minimax-m"))
            })
        })
        .or_else(|| {
            models
                .iter()
                .filter(available)
                .find(|model| is_text_model(model))
        })
}
...
fn used_percent(
    ...
    status_field: &str,
    remaining_percent_field: &str,
    total_field: &str,
    count_field: &str,
) -> f64 {
    match token_plan_status(model, status_field) {
        2 => 100.0,
        3 => 0.0,
        _ => {
            let remaining_percent = json_number(model.get(remaining_percent_field)).or_else(|| {
                let total = token_plan_total(model, total_field);
                (total > 0.0).then(|| token_plan_total(model, count_field) / total * 100.0)
            });
            100.0 - remaining_percent.unwrap_or(100.0)
        }
    }
}

fn millis_timestamp(model: &serde_json::Value, field: &str) -> Option<DateTime<Utc>> {
    let millis = json_number(model.get(field))? as i64;
...
fn parse_token_plan_response(json: &serde_json::Value) -> Result<UsageSnapshot, ProviderError> {
    let models = json
        ...
        .ok_or_else(|| {
            ProviderError::Parse("MiniMax Token Plan response has no model_remains".into())
        })?;
    let model = select_token_plan_model(models).ok_or_else(|| {
        ProviderError::Other(
            "MiniMax text models are not included in the current Token Plan".into(),
        )
    })?;
    let name = model
        .get("model_name")
        .and_then(serde_json::Value::as_str)
        .unwrap_or("MiniMax Token Plan");
    let primary = RateWindow::with_details(
        ...
        Some(300),
        millis_timestamp(model, "end_time"),
        None,
    );
    let mut usage = UsageSnapshot::new(primary).with_login_method(name);
    let weekly_status = token_plan_status(model, "current_weekly_status");
    if weekly_status == 3 {
        usage = usage.with_extra_rate_window(
            UNLIMITED_WEEK_MARKER_ID,
            UNLIMITED_WEEK_MARKER_ID,
            RateWindow::informational(""),
        );
    } else if matches!(weekly_status, 1 | 2) {
        usage = usage.with_secondary(RateWindow::with_details(
            ...
            Some(7 * 24 * 60),
            millis_timestamp(model, "weekly_end_time"),
            None,
        ));
    }
    Ok(usage)
...
impl Provider for MiniMaxProvider {
    ...
    async fn fetch_usage(&self, ctx: &FetchContext) -> Result<ProviderFetchResult, ProviderError> {
        ...
        if let Some(api_key) = ctx.api_key.as_deref() {
            return self.fetch_token_plan(api_key, region).await;
        }
...
mod tests {
    ...
    #[test]
    fn minimax_region_defaults_to_china_mainland_urls() {
    ...
    #[test]
    fn parses_finite_token_plan_windows_and_prefers_general_model() {
        let json = serde_json::json!({
            "model_remains": [
                ...
                },
                {
                    ...
                    "weekly_end_time": 1_800_086_400_000_i64
                }
            ]
        });

        let usage = parse_token_plan_response(&json).expect("token plan");

        assert_eq!(usage.primary.used_percent, 75.0);
        assert_eq!(usage.primary.window_minutes, Some(300));
        assert_eq!(
            usage.primary.resets_at.unwrap().timestamp_millis(),
        ...
        assert_eq!(
            weekly.resets_at.unwrap().timestamp_millis(),
            1_800_086_400_000_i64
        );
    }

    #[test]
    fn marks_unlimited_week_without_rendering_a_weekly_quota() {
        let json = serde_json::json!({
            "model_remains": [{
        ...
        assert_eq!(usage.extra_rate_windows[0].id, "minimax-weekly-unlimited");
    }

    #[test]
    fn does_not_infer_a_weekly_quota_when_weekly_status_is_missing() {
        let json = serde_json::json!({
            "model_remains": [{
                "model_name": "general",
                "current_interval_status": 1,
    ...
    #[test]
    fn rejects_model_bucket_that_is_not_in_the_current_plan() {
        let json = serde_json::json!({
            "model_remains": [{
                "model_name": "general",
        ...
        let error = parse_token_plan_response(&json).expect_err("unavailable model");
        assert!(
            error
                .to_string()
                .contains("not included in the current Token Plan")
        );
    }
