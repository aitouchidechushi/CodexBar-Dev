...
pub(crate) fn resolve_factory_api_key(
    ctx_key: Option<&str>,
) -> Result<Option<String>, ProviderError> {
    if let Some(explicit) = clean_factory_secret(ctx_key) {
    ...
    let api_keys = ApiKeys::load()
        .and_then(|state| state.into_writable_for(StoreKind::ApiKeys))
        .map_err(|error| ProviderError::Other(format!("API key storage unavailable: {error}")))?;
    let saved = api_keys.get("factory").map(str::to_string);
    Ok(resolve_factory_api_key_from(None, &env, saved.as_deref()))
}
...
impl FactoryProvider {
    ...
    async fn fetch_via_api(&self, api_key: &str) -> Result<UsageSnapshot, ProviderError> {
        ...
        let (billing, api_auth) = tokio::join!(
            self.fetch_billing_limits(&client, api_key),
            self.fetch_auth_info(&client, FACTORY_API_BASE, None, Some(api_key)),
        );
    ...
    async fn fetch_auth_and_usage_bearer(
        ...
        let (auth_info, usage_data) = tokio::join!(
            self.fetch_auth_info(client, base, None, Some(api_key)),
            self.fetch_usage_api(client, base, None, Some(api_key)),
        );
    ...
    async fn fetch_via_web(&self, ctx: &FetchContext) -> Result<UsageSnapshot, ProviderError> {
        ...
        let client = Self::build_client()?;
        let (auth_info, usage_data) = tokio::join!(
            self.fetch_auth_info(&client, FACTORY_APP_BASE, Some(&cookies), None),
            self.fetch_usage_api(&client, FACTORY_APP_BASE, Some(&cookies), None),
        );
    ...
    fn apply_auth_info(
        ...
        if let Some(user_id) = user_id {
            usage = usage.with_account_group_id(format!("factory:{user_id}"));
        }
...
fn snapshot_from_billing_limits(
    ...
    let tertiary = limits.standard.monthly.rate_window(Some(43_200));
        ...
        && core.has_usage_data()
    {
        for (id, title, window) in [
            ...
            (
                ...
                "Core Monthly",
                core.monthly.rate_window(Some(43_200)),
            ),
        ] {
...
mod tests {
    ...
    #[test]
    fn task_2a_factory_user_id_sets_group_and_safe_account_fields() {
        let auth: FactoryAuthResponse = serde_json::from_str(
            r#"{
                "user": {
                    "id": "usr_42",
                    "name": "Ada Lovelace",
        ...
        let snapshot =
            FactoryProvider::apply_auth_info(UsageSnapshot::new(RateWindow::new(18.0)), Some(auth));

        assert_eq!(snapshot.account_group_id.as_deref(), Some("factory:usr_42"));
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("Factory team - Enterprise")
        );
    }

    #[test]
    fn task_2a_factory_absent_auth_keeps_usage_ungrouped() {
        let snapshot =
            FactoryProvider::apply_auth_info(UsageSnapshot::new(RateWindow::new(63.0)), None);

        assert_eq!(snapshot.primary.used_percent, 63.0);
        assert!(snapshot.account_group_id.is_none());
        assert_eq!(snapshot.login_method.as_deref(), Some("Droid"));
    }

    #[test]
    fn task_2a_factory_standard_and_present_core_months_are_43200_minutes() {
        let body = r#"{
          "usesTokenRateLimitsBilling": true,
          "limits": {
            "standard": {
            ...
            "core": {
              ...
              "monthly": { "usedPercent": 9, "secondsRemaining": 1209600 }
            }
          }
        }"#;
        let parsed: FactoryBillingLimitsResponse = serde_json::from_str(body).unwrap();
        let snapshot = snapshot_from_billing_limits(&parsed.limits.unwrap(), None);
        let core_month = snapshot
            .extra_rate_windows
            .iter()
            .find(|window| window.id == "factory-core-monthly")
            .unwrap();
        ...
        assert_eq!(core_month.window.window_minutes, Some(43_200));
    }

    #[test]
    fn task_2a_factory_legacy_windows_remain_without_month_duration() {
        let parsed: FactoryUsageResponse = serde_json::from_str(
            r#"{
              "standard": { "used": 25.0, "allowance": 100.0 },
              "premium": { "used": 10.0, "allowance": 50.0 }
            }"#,
        ...
        .unwrap();
        let snapshot = FactoryProvider::usage_snapshot_from_response(&parsed);

        assert_eq!(snapshot.primary.window_minutes, None);
        assert_eq!(snapshot.secondary.as_ref().unwrap().window_minutes, None);
        assert!(snapshot.tertiary.is_none());
    }

    #[test]
    fn task_2a_factory_sources_keep_api_first_and_web_fallback_modes() {
        let sources = FactoryProvider::new().available_sources();
