...
#[derive(Debug, Deserialize)]
struct ZaiLimit {
    /// Limit type: "CREDIT_LIMIT", "TOKENS_LIMIT", or "TIME_LIMIT" (upstream),
    /// or "tokens"/"mcp" (legacy).
    #[serde(rename = "type")]
...
impl ZaiProvider {
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                id: ProviderId::Zai,
                display_name: ProviderId::Zai.display_name(),
                session_label: "Rate Limit",
                weekly_label: "Weekly",
                supports_opus: false,
    ...
    fn parse_quota_response(
        ...
        // GLM Coding Plan currently reports quota windows as CREDIT_LIMIT.
        // Keep TOKENS_LIMIT and legacy tokens support for older responses.
        let mut model_limits: Vec<&ZaiLimit> = limits
            .iter()
            .filter(|l| {
                matches!(
                    l.limit_type.as_deref(),
                    Some("CREDIT_LIMIT") | Some("TOKENS_LIMIT") | Some("tokens")
                )
            })
            .collect();

        // TIME_LIMIT and legacy mcp entries are tool quotas. The model type allowlist above
        // intentionally keeps them out of every generic UsageSnapshot quota slot.
        if model_limits.is_empty() {
            return Err(ProviderError::Parse(
                "z.ai response did not contain a model quota limit".to_string(),
            ));
        }

        // Sort model limits by window_minutes: shortest first.
        model_limits.sort_by_key(|l| Self::window_minutes(l));
        ...
        // Build model windows using the shared UsageSnapshot contract:
        // shortest → primary/session (5-hour), longest → secondary/weekly.
        let (primary, secondary) = match model_limits.len() {
            1 => {
                let p = make_window(model_limits[0], Self::window_minutes(model_limits[0]));
                (p, None)
            }
            _ => {
                let session = model_limits.first().unwrap();
                let weekly = model_limits.last().unwrap();
                let p = make_window(session, Self::window_minutes(session));
                let s = Some(make_window(weekly, Self::window_minutes(weekly)));
                (p, s)
            }
        };
...
mod tests {
    ...
    #[test]
    fn parses_credit_limit_five_hour_and_weekly_windows() {
        let provider = ZaiProvider::new();
        let quota: ZaiQuotaResponse = serde_json::from_value(serde_json::json!({
            "code": 200,
            "data": {
                ...
                "limits": [
                    {
                        ...
                        "remaining": 26194,
                        "percentage": 6,
                        "nextResetTime": 1788134400000_i64
                    },
                    {
                        "type": "CREDIT_LIMIT",
                        "unit": 6,
                        ...
                        "remaining": 15344,
                        "percentage": 89,
                        "nextResetTime": 1788739200000_i64
                    }
                ]
            }
        }))
        ...
        assert_eq!(provider.metadata.session_label, "Rate Limit");
        assert_eq!(provider.metadata.weekly_label, "Weekly");
        assert_eq!(usage.primary.used_percent, 6.0);
        assert_eq!(usage.primary.window_minutes, Some(300));
        assert!(usage.primary.resets_at.is_some());
    ...
    #[test]
    fn mixed_credit_and_time_limits_only_emit_model_quota_windows() {
        let provider = ZaiProvider::new();
        let quota: ZaiQuotaResponse = serde_json::from_value(serde_json::json!({
            "code": 200,
            "data": {
        ...
        assert_ne!(usage.primary.window_minutes, Some(43_200));
        assert_ne!(weekly.window_minutes, Some(43_200));
    }

    #[test]
    fn tool_time_limits_without_model_limits_return_parse_error() {
        let provider = ZaiProvider::new();
        let fixtures = [
            serde_json::json!({
                "code": 200,
        ...
        for fixture in fixtures {
            let quota: ZaiQuotaResponse = serde_json::from_value(fixture).unwrap();
            let error = provider.parse_quota_response(&quota).unwrap_err();

            assert!(matches!(
                error,
                ProviderError::Parse(message) if message.contains("model quota")
            ));
        }
    }

    #[test]
    fn available_sources_remain_auto_and_oauth_only() {
        assert_eq!(
            ZaiProvider::new().available_sources(),
            vec![SourceMode::Auto, SourceMode::OAuth]
        );
    }
