...
mod tests {
    ...
    #[test]
    fn missing_limit_does_not_use_a_fabricated_fallback() {
        let usage = AmpProvider::new()
    ...
    #[test]
    fn real_limit_requires_explicit_month_semantics_for_hard_month() {
    ...
    #[test]
    fn invalid_amp_numbers_do_not_create_hard_months() {
        let provider = AmpProvider::new();
        let fixtures = [
            json!({"used": 25, "limit": "NaN", "cycle": "month"}),
            json!({"used": 25, "limit": 0, "cycle": "month"}),
            json!({"used": "NaN", "limit": 100, "interval": "30-day"}),
        ];
