...
当前源码与已验收程序版本为 **0.48.2 / build 92**。用户于 2026-09-22 确认验收通过；更新源仍未配置。

[下载已验收的 0.48.2 便携版](https://github.com/aitouchidechushi/CodexBar-Dev/releases/tag/v0.48.2)，在 Assets 中选择 `CodexBar-0.48.2-portable.exe`，不是 Source code。这是用户验收的同一份 Windows x64 Release 构建，内嵌界面，无需开发服务器；需要 WebView2 Runtime。启动前请从旧程序菜单选择“退出”。源码标签 `v0.48.2` 对应本次发布源码，Release 附件为原验收文件，不另行重编译。

EXE SHA256：`B3940EE3E5665A5DE230B8DD3E357C1571B154739116F002F5A950560C48511F`。

此 EXE 未签名，Windows 可能提示发布者未知。请核对下载来源及上述哈希；不建议关闭系统安全防护。

本次新增 Kimi、MiniMax、GLM 全部启用 Key 的统一并发检测。点击按钮直接开始，可能消耗套餐额度或余额；无二次确认，不自动重试。主窗口按钮位于“刷新全部”左侧，结果在各供应商失败计数旁悬停查看，明确受限的 Key 保留异常提示。响应重叠不是账号真实并发上限；普通限流、权限和额度错误不当成并发限制。结果仅本次进程内保留，退出后清除。GPT/Codex 等暂不检测。

包含此前额度刷新可靠性、安装覆盖保护，以及相同路径和相同文件的便携版重复启动交接修复。前端 360 项测试通过；最近完整 Rust 回归 1,495 项通过，版本、类型检查与 Release 编译通过。用户验收不代表所有功能或所有电脑兼容性都已验证。

已知未完成项：更新检查尚未区分部分请求失败与无新版；真实安装/升级/卸载、历史配置跨机器迁移和旧新进程交接仍需验证。不同路径、不同内容或身份不明的旧进程仍会被安全地阻止自动接管，需用户手动退出。PTY 曾出现全套测试中的间歇失败，不能以某次通过声称根治。已分发的旧安装包不会自动获得新保护。
